...
## 2024-05-19 - Adding ARIA labels to Commit/Cancel/Reset IconButtons in Adjust Move Crop Panels
**Learning:** Found that some `IconButton` elements used for Commit, Cancel, and Reset actions within the crop/adjust panels (`adjustMoveCropPanels.tsx`) lacked `aria-label`s, rendering them inaccessible to screen readers despite having visual `<Tooltip>` wrappers. Also, tooltips failed to display on disabled buttons (like "Apply crop" or "Cancel crop preview" when `hasPendingCrop` is false).
**Action:** Always add an explicit `aria-label` matching the tooltip text onto standard Material-UI `IconButton` elements. When wrapping a potentially `disabled` Material-UI component inside a `<Tooltip>`, wrap the disabled component in a `<span style={{ display: 'inline-flex' }}>` tag to preserve the intended layout while capturing pointer events so the tooltip can appear.
