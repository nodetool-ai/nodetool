 ...
 *
 * There is no regeneration script in this repo — the snapshot is committed by
 * hand, and only `components["schemas"]` is consumed (`src/services/api.ts`).
 * When an endpoint moves to tRPC, delete its `paths` and `operations` entries
 * here in the same change, or this file describes a contract the server no
 * longer serves.
 */
