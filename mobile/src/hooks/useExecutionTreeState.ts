 ...
 *
 * Ported from web/src/hooks/useExecutionTreeState.ts so the mobile app can
 * render the same agent execution tree (plan/tasks/steps) as the web UI.
 */
...
import type { StepToolCall } from '../stores/ChatStore';
...
function normalizeContent(msg: Message): {
  ...
  let eventType = (msg as Message & { execution_event_type?: string | null })
...
export function buildExecutionTreeState(
  ...
  for (const msg of messages) {
    ...
    if (eventType === 'log_update') {
      ...
      state.logs.push({
        nodeId: lu.node_id ?? '',
        content:
          typeof lu.content === 'string'
            ? lu.content
            : JSON.stringify(lu.content),
    ...
    if (eventType === 'tool_call_update') {
      const tc = content as ToolCallUpdate & { step_id?: string | null };
  ...
  // Attach tool calls from the chat store so each step shows the last tool run
  if (toolCallsByStep) {
    for (const task of taskMap.values()) {
...
export function useExecutionTreeState(
  ...
  // Apply user expand/collapse overrides on top of default per-task state
  ...
  const toggleExpand = useCallback((taskId: string) => {
    setExpandOverrides((prev) => {
      ...
      // If no override yet, flip the default (which is true for running tasks,
      // false for completed tasks).
