...
class BeatTracking(BaseNode):
    """
    Perform beat tracking on an audio file.
    audio, rhythm, tempo, beats
    ...
    audio: AudioRef = Field(
        default=AudioRef(), description="The audio file to analyze for beats."
    )
    start_bpm: float = Field(
        default=120.0,
        ge=60.0,
        le=300.0,
        description="The starting tempo estimate in beats per minute.",
    )

    @classmethod
    def get_return_type(cls) -> dict:
        return {
            "tempo": float,
            "beat_frames": Tensor,
            "beat_times": Tensor,
    ...
    async def process(self, context: ProcessingContext) -> dict:
        ...
        tempo, beat_frames = librosa.beat.beat_track(
            y=samples, sr=sample_rate, start_bpm=self.start_bpm, units="frames"
        )
        ...
        return {
            "tempo": tempo,
            "beat_frames": Tensor.from_numpy(beat_frames),
            "beat_times": Tensor.from_numpy(beat_times),
        }


class HarmonicPercussiveSeparation(BaseNode):
    """
    Perform Harmonic-Percussive Source Separation on an audio file.
    audio, separation, harmonic, percussive
    ...
    audio: AudioRef = Field(
        default=AudioRef(),
        description="The audio file to separate into harmonic and percussive components.",
    )
    margin: float = Field(
        default=1.0,
        ge=0.0,
        le=10.0,
        description="The margin for the separation (larger values increase separation at the cost of artifacts).",
    )
    ...
    async def process(self, context: ProcessingContext) -> dict[str, AudioRef]:
        ...
        harmonic, percussive = librosa.effects.hpss(samples, margin=self.margin)

        # Convert separated components back to AudioRef
        harmonic_audio = await context.audio_from_numpy(
            harmonic, sample_rate, num_channels=1, name="harmonic.wav"
        )
        percussive_audio = await context.audio_from_numpy(
            percussive, sample_rate, num_channels=1, name="percussive.wav"
        )

        return {"harmonic": harmonic_audio, "percussive": percussive_audio}
