...
class BAE_Normal_Map_Preprocessor(GraphNode):
    ...
    comfy_class: str | GraphNode | tuple[GraphNode, str] = Field(default='BAE-NormalMapPreprocessor', description=None)
    @classmethod
...
class InpaintPreprocessor(GraphNode):
    image: ImageTensor | GraphNode | tuple[GraphNode, str] = Field(default=ImageTensor(type='comfy.image_tensor'), description='The image to inpaint.')
    resolution: int | GraphNode | tuple[GraphNode, str] = Field(default=512, description='The width of the image to generate.')
    mask: Mask | GraphNode | tuple[GraphNode, str] = Field(default=Mask(type='comfy.mask'), description='The mask to use for inpainting.')
    @classmethod
...
class MeshGraphormerDepthMapPreprocessor(GraphNode):
    image: ImageTensor | GraphNode | tuple[GraphNode, str] = Field(default=ImageTensor(type='comfy.image_tensor'), description='The image to preprocess.')
    resolution: int | GraphNode | tuple[GraphNode, str] = Field(default=512, description='The width of the image to generate.')
    mask_bbox_padding: int | GraphNode | tuple[GraphNode, str] = Field(default=30, description='The padding for the mask bounding box.')
    @classmethod
    def get_node_type(cls): return "comfy.controlnet.normal_and_depth.MeshGraphormerDepthMapPreprocessor"
