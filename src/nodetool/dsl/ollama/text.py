...
class Embedding(GraphNode):
    input: str | nodetool.metadata.types.TextRef | GraphNode | tuple[GraphNode, str] = (
        Field(default="", description=None)
    )
    model: LlamaModel | GraphNode | tuple[GraphNode, str] = Field(
        default=LlamaModel(
            type="llama_model",
            name="",
            repo_id="",
    ...
    chunk_size: int | GraphNode | tuple[GraphNode, str] = Field(
        default=4096, description="The size of the chunks to split the input into"
    )

    @classmethod
    def get_node_type(cls):
        return "ollama.text.Embedding"
...
class Ollama(GraphNode):
    ...
    prompt: str | GraphNode | tuple[GraphNode, str] = Field(
        default="", description="Prompt to send to the model."
    )
    ...
    temperature: float | GraphNode | tuple[GraphNode, str] = Field(
        default=0.7, description="The temperature to use for the model."
    )
