...
class ClarityUpscaler(GraphNode):
    ...
    sd_model: nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Sd_model = Field(default=nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Sd_model('juggernaut_reborn.safetensors [338b85bc4f]'), description='Stable Diffusion model checkpoint')
    scheduler: nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Scheduler = Field(default=nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Scheduler('DPM++ 3M SDE Karras'), description='scheduler')
    ...
    tiling_width: nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Tiling_width = Field(default=nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Tiling_width(112), description='Fractality, set lower tile width for a high Fractality')
    ...
    tiling_height: nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Tiling_height = Field(default=nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Tiling_height(144), description='Fractality, set lower tile height for a high Fractality')
...
class GFPGAN(GraphNode):
    ...
    version: nodetool.nodes.replicate.image.upscale.GFPGAN.Version = Field(default=nodetool.nodes.replicate.image.upscale.GFPGAN.Version('v1.4'), description='GFPGAN version. v1.3: better quality. v1.4: more details and better identity.')
...
class HighResolutionControlNetTile(GraphNode):
    ...
    resolution: nodetool.nodes.replicate.image.upscale.HighResolutionControlNetTile.Resolution = Field(default=nodetool.nodes.replicate.image.upscale.HighResolutionControlNetTile.Resolution(2560), description='Image resolution')
...
class SwinIR(GraphNode):
    ...
    image: ImageRef | GraphNode | tuple[GraphNode, str] = Field(default=ImageRef(type='image', uri='', asset_id=None, data=None), description='input image')
    noise: nodetool.nodes.replicate.image.upscale.SwinIR.Noise = Field(default=nodetool.nodes.replicate.image.upscale.SwinIR.Noise(15), description='noise level, activated for Grayscale Image Denoising and Color Image Denoising. Leave it as default or arbitrary if other tasks are selected')
    task_type: nodetool.nodes.replicate.image.upscale.SwinIR.Task_type = Field(default=nodetool.nodes.replicate.image.upscale.SwinIR.Task_type('Real-World Image Super-Resolution-Large'), description='Choose a task')
...
class UltimateSDUpscale(GraphNode):
    ...
    upscaler: nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Upscaler = Field(default=nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Upscaler('4x-UltraSharp'), description='Upscaler')
...
class ruDallE_SR(GraphNode):
    ...
    image: ImageRef | GraphNode | tuple[GraphNode, str] = Field(default=ImageRef(type='image', uri='', asset_id=None, data=None), description='Input image')
    scale: nodetool.nodes.replicate.image.upscale.ruDallE_SR.Scale = Field(default=nodetool.nodes.replicate.image.upscale.ruDallE_SR.Scale(4), description='Choose up-scaling factor')
