---
name: ast-grep
description: "Search code by AST structure with ast-grep when text search cannot express the required pattern."
---
...
The CLI is not a repo dependency — run it via:

```bash
npx --yes --package @ast-grep/cli ast-grep <args>
```

(A bare `npx @ast-grep/cli` fails with "could not determine executable to
run" — the binary is named `ast-grep`, not after the package.) Use `--lang
tsx` for the React frontend. To update this skill, re-clone upstream and
re-copy `ast-grep/skills/ast-grep/` here, keeping this section.
