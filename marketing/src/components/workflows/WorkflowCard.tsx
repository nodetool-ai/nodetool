...
export default function WorkflowCard({ workflow }: WorkflowCardProps) {
  return (
    <Link
      ...
      className="group relative flex flex-col overflow-hidden rounded-2xl border border-neutral-800/70 bg-neutral-900/60 ring-1 ring-white/5 transition-all hover:border-blue-500/50 hover:ring-blue-400/20 hover:-translate-y-0.5"
      aria-label={`Open the ${workflow.title} workflow`}
    >
      <div className="relative h-56 w-full border-b border-neutral-800/70 bg-gradient-to-br from-neutral-950 to-neutral-900">
        <WorkflowPreviewFlow
        ...
        <div className="absolute left-4 top-4 inline-flex items-center gap-1.5 rounded-md bg-black/60 px-2 py-1 text-[10px] font-medium uppercase tracking-wider text-neutral-300 ring-1 ring-white/10 backdrop-blur">
          {workflow.category}
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        <p className="text-sm text-neutral-400 leading-relaxed line-clamp-3">{workflow.tagline}</p>
