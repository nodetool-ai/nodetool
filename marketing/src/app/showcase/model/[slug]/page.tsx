...
import { showcaseEntries, showcaseModels, humanize } from "@/data/showcase";
import { ShowcaseGrid } from "../../_components";

const BASE_URL = "https://nodetool.ai";

export function generateStaticParams() {
  return showcaseModels().map((m) => ({ slug: m.slug }));
}

function entriesForModel(slug: string) {
  return showcaseEntries.filter((e) => e.modelSlug === slug);
}
...
export default async function ShowcaseModelPage({
  ...
  const { slug } = await params;
  const entries = entriesForModel(slug);
      ...
      <div className="relative isolate pt-28 sm:pt-36">
        <section className="mx-auto max-w-6xl px-6">
          ...
          <p className="mt-4 max-w-2xl text-lg text-slate-300">
            {entries.length} generation{entries.length === 1 ? "" : "s"} made with{" "}
            {label} on NodeTool.
          </p>
        </section>
