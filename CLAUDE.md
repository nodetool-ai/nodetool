...
`ApplicationBundle`, without touching the database. Five stages run in order:
...
wiring against those graphs, and **run** replays every interaction on the
kernel and asserts what each widget ends up showing. A judge stage is stubbed —
it lands next, and until then the verdict says every interaction was recorded as
achieved without being scored.
...
--judge-model <provider/model>            judge model (default NODETOOL_APP_JUDGE_MODEL)
...
Harness code: `packages/agents/src/app-build/` (`buildApp`, the spec/author
