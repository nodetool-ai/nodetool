...
- **Packaged Electron backend flattens file paths**. esbuild bundles the backend into one `server.mjs`, so anything resolved relative to `import.meta.url` (provider `*-manifest.json`, examples, `package://` assets) must be staged next to it by `electron/scripts/bundle-backend.mjs` — in dev these resolve via package resolution, so gaps only surface in the packaged app. `electron/scripts/verify-backend-bundle.mjs` (run automatically after bundling) fails the build on any missing file. See [electron/src/AGENTS.md § Packaged file layout](electron/src/AGENTS.md).
