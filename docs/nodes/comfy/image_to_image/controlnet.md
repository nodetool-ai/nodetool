...
| grow_mask_by | `any` |  | `6` |
| denoise | `any` |  | `1.0` |
| controlnet | `any` | The ControlNet model to use. | `{'type': 'hf.controlnet', 'repo_id': '', 'path': None, 'variant': None, 'allow_patterns': None, 'ignore_patterns': None}` |
| image | `any` | Canny edge detection image for ControlNet | `{'type': 'image', 'uri': '', 'asset_id': None, 'data': None}` |
| strength | `any` | Strength of ControlNet (used for both low and high resolution) | `1.0` |
