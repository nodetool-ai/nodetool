...
|----------|------|-------------|----------|
| model | `any` | The model ID to use for the classification | `{'type': 'hf.zero_shot_image_classification', 'repo_id': '', 'path': None, 'variant': None, 'allow_patterns': None, 'ignore_patterns': None}` |
| image | `any` | The input image to classify | `{'type': 'image', 'uri': '', 'asset_id': None, 'data': None}` |
| candidate_labels | `any` | The candidate labels to classify the image against, separated by commas | `` |
