...
- **Agent system**: `authorGraph` authors runnable workflow graphs from an objective (`packages/agents/src/author-graph.ts`); media tools (`generate_video`, `animate_image`, `generate_speech` in `packages/agents/src/tools/media-tools.ts`); `debug_workflow` and `validate_workflow` for self-correction; the chat agent edits the live canvas through `ui_*` tools.
