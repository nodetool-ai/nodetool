...
Nodes are organized by category across the node packages — `packages/core-nodes/` (constants, control flow, math), `packages/text-nodes/`, `packages/image-nodes/`, `packages/llm-nodes/`, `packages/automation-nodes/`, `packages/integration-nodes/`, and others. (The old `packages/base-nodes/src/nodes/` directory is gone — `packages/base-nodes/src/index.ts` only re-exports and aggregates, into `ALL_BASE_NODES`.) Each file exports an array of node classes (e.g., `CONTROL_NODES`, `TEXT_EXTRA_NODES`) and the package index registers them all. The system automatically discovers all registered classes inheriting from **`BaseNode`**.
