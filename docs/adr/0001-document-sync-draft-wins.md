...
- `web/src/stores/documentSync.ts` is the canonical editor synchronization
  seam. Each editor supplies its document adapter while the shared module owns
  debounce, serialization, flush, compare-and-swap recovery, and routing of
  external changes. The checked inventory is in
  [`docs/document-sync.md`](../document-sync.md).
...
- A shared editor controller that meets a CAS rejection reloads the latest
  document before it retries a still-dirty draft. Headless capability retry
  behavior remains specific to the capability that issued the write.
