...
3. **Run autonomous agents from command line**  
   Use the [Agent CLI](agent-cli.md) to run agents with YAML configurations for fully automated task execution.

4. **Expose agents via APIs or CLI**  
   Use the [Chat CLI](chat-cli.md) for interactive automation scripts, or call the [Chat API](chat-api.md) from backend services so agents can operate in headless environments.

5. **Stream updates back into editors**  
