...
- **A5.** Add proxies (F11). A video whose longer side is at least `NODETOOL_VIDEO_PROXY_MIN_SIZE_PX` (setting or env, default 1920, `0` for every video, negative for none) gets a proxy in the background after upload, `createExternal`, or relink. `assets.ensureProxy({ id })` asks for one at any size and retries a failure. One in-process queue encodes one video at a time, so an import returns before the encode starts.
  - The proxy is H.264 with every frame a keyframe and no B-frames, scaled so the longer side is at most 960 px, in MP4. ProRes and DNxHR do not decode in Chromium, and MJPEG does not play in `<video>`. An ffmpeg without libx264 writes all-intra VP9 in WebM instead. `-fps_mode passthrough` and the source's track timescale keep every frame at its source timestamp, so a clip's source times need no mapping. The proxy has no audio, because preview video elements are muted and sound plays from audio clips.
  - The proxy is a sidecar in the asset store, `<owner>/<id>_proxy.mp4`, not an asset row. A manifest, `<owner>/<id>_proxy.json`, records the source file's size and mtime. A row would need a filter in every listing, search, agent tool, and export, and a sidecar is invisible to all of them, as the thumbnail is. The runtime's short-id listing fallback skips `_proxy` files, so a render never resolves an id to the proxy. Asset delete, project delete, and relink remove the proxy and stop its encode.
  - The asset response carries `proxy_status` (`none`, `queued`, `running`, `ready`, `failed`) and, when ready, `proxy_url` with `?v=` from the source version. A proxy made from another version of the file is not ready (R2). The timeline preview plays `proxy_url` when ready and polls a pending asset until it is. The browser export (`getAssetUrl`), filmstrips, duration probes, and the server render keep reading the original.
  - Local mode only, gated like D6. A cloud deployment would need a worker to encode and an object-store upload.
