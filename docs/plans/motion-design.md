...
`(params, canvas, role, durationMs?) => PropertyCurve[]`. `durationMs` is the
animation's own window length, for a preset whose motion depends on it (a
noise fade envelope, arc-length sample density) — optional, so a preset that
ignores it keeps a shorter signature. `AnimationPresetId` is the union of ids.
`sampleCountFor(durationMs, frequencyHz, {min, max})` is the shared sample
density rule (~8 keyframes per cycle, capped at `MAX_CUSTOM_KEYFRAMES`) that
`shake`, `float`, and `followPath` all use instead of a fixed count.
...
| `shake`    | emphasis | `intensity` (0.02, fraction of canvas width/height), `frequency` (8 Hz), `seed` (1), `fadeInMs`/`fadeOutMs` (0) | seeded value noise (mulberry32, deterministic per seed) on both offsetX and offsetY, amplitude-enveloped by `fadeInMs`/`fadeOutMs`; the curve's own first/last keyframes are always forced to 0 regardless of fade settings |
...
| `float`    | loop     | `amplitude` (0.015), `frequency` (1 cycle/period), `seed` (0 = pure sine)                  | offsetY sine bob (loops seamlessly — the sine always closes on a whole cycle regardless of a fractional `frequency`); a nonzero `seed` blends in seeded circular value noise as organic drift without breaking the loop                              |
| `breathe`  | loop     | `intensity` (0.03)                                                                         | scale sine                                                                                                                                                                                                                                            |
| `rotate`   | loop     | `rpm` via durationMs                                                                       | rotation 0→2π per cycle                                                                                                                                                                                                                               |
| `followPath` | emphasis, loop | `d` (SVG path, normalized 0..1 like `ClipShapeStyle.d`), `pathX`/`pathY`/`pathWidth`/`pathHeight` (0,0,1,1 — normalized placement box), `orient` (false), `startT`/`endT` (0, 1) | walks `d` by arc length from `startT` to `endT`, writing `positionX`/`positionY` (replace, canvas px) and, when `orient` is set, `rotation` from the path's tangent. Takes the path literally — resolving an existing shape clip's `d` into these params (`pathClipId`) is a follow-up op, since a preset compiles without the document to look one up in. Geometry: `pathSampling.ts`, reusing `render/shapeGeometry.ts`'s flattener |
