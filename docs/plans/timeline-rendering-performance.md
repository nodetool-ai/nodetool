...
This plan targets four separate costs: browser frame scheduling, decoder readiness at cuts, headless readback, and preview memory. Keep preview, browser export, server export, and agent frame preview on the shared scene and compositor rules in [`packages/timeline/AGENTS.md`](https://github.com/nodetool-ai/nodetool/blob/main/packages/timeline/AGENTS.md#rendering-srcrender-nodetool-aitimelinerender). Preserve the transient playback clock, scene change horizon, source texture reuse, batched frame compositing, and existing multi-sample GPU alpha resolve.

## Delivery rules

- Start each task from the current branch and read [`AGENTS.md`](../../AGENTS.md), the relevant area overlays, and [`docs/DEVELOPMENT_STANDARDS.md`](https://github.com/nodetool-ai/nodetool/blob/main/docs/DEVELOPMENT_STANDARDS.md). This plan describes work, not an exception to those rules.
...
|---|---|---|---|---|
| L0 | Sol | none | New fixture/report files under `web/src/components/timeline/` or `web/src/perf/`, plus minimal diagnostic hooks in `preview/gpu/compositor.ts` | Core baseline, failure reproductions, report contract |
| L3 | Sol | L0 | The L0 fixture and report files | Extended scenario matrix and export baseline |
...
| S4 | Sol | S3, L3 | `frameCompositor.ts`, `packages/video-nodes/src/nodes/timeline/compositeRender.ts`, `rawFrames.ts` | Bounded two-frame render/readback and ordered encoding |
| L1 | Sol | L3, S1, S2 | `preview/textRender.ts`, `preview/shapeRender.ts`, `preview/PreviewCompositor.tsx`, `render/TimelineRenderer.ts`, cache tests | Byte-bounded bitmap caches with frame-safe eviction |
| L2 | Sol | S2, L1 | `preview/PreviewArea.tsx`, `preview/PreviewCompositor.tsx`, `web/src/stores/timeline/TimelineUIStore.ts` | Auto/Full/Half/Quarter control and correct backing resolution |
...
| S6 | Sol | L3, S2, L1 | Browser export and decode files, assigned after codec probe | Bounded timestamped decode for sequential browser export |
| S7 | Sol | L3, S6 | Browser scrub request files, assigned after trace | Coalesced scrub requests |
| S8 | Sol | L3, S4 | `packages/video-nodes/src/nodes/timeline/rawFrames.ts` | Bounded server reverse GOP windows |

Sol agents own the fixture, cache, UI, scheduler, decoder, and GPU changes. Test files belong to the task that changes their behavior. The L0 diagnostic hook merges before S1 edits `preview/gpu/compositor.ts`. L1 waits for S2 because it touches `PreviewCompositor.tsx`; L2 waits for L1 for the same reason. S8 waits for S4 because both edit `rawFrames.ts`. S3 can run alongside L0 because its files do not overlap, but its performance claim waits for L3's export baseline. A workable two-agent order is L0 with S3, L3 with S1, S2, L1 with S4, then L2 with S6.

### L0 — Browser baseline and reproductions

Build a runnable scenario against the real preview, with a JSON report and a short command documented next to it. Use `requestVideoFrameCallback` metadata and `getVideoPlaybackQuality` where available. Add minimal source-upload and compositor-submission diagnostic hooks at the existing sites in `preview/gpu/compositor.ts`, behind a flag; S1 will retain these hooks when changing that file. Capture the current redundant-work behavior during steady 24 fps playback on a high-refresh display, then capture the 60-second cut and trimmed in-point. Keep the metric collector outside React state and disabled for ordinary playback.
...
Track a monotonically increasing decoded-frame generation per active preview `HTMLVideoElement` using `requestVideoFrameCallback`, with callback cleanup on slot reuse and unmount. Give the uploader a source-version interface for tracked preview elements. Browser export uses the same uploader with sought elements; keep a deterministic seek-based version path for those elements, including source replacement at the same timestamp, until S6 changes decode. Reuse a GPU texture when a new bitmap has the same dimensions and format, but upload its changed pixels. Composite when a video generation changes or when scene changes, animation, transitions, motion, caption timing, or the playhead requires it. Keep an explicit fallback for browsers without the callback. A seek requests redraw when the correct decoded frame arrives, not merely when `currentTime` changes.

**Acceptance:** A held video frame causes no repeat upload or composite solely because rAF ticked. Animated overlays still composite on their own clock. Pause, seek, source replacement, loop, cut, matte, and callback-unavailable cases show a correct frame. Browser export still uploads each sought frame in sequential, retimed, matte, and motion-blur cases, including a replaced source at the same timestamp. L0 reports fewer uploads and submissions in steady video playback without more wrong, dropped, or duplicate source frames. Add focused tests near [`preview/gpu/__tests__/compositor.test.ts`](https://github.com/nodetool-ai/nodetool/blob/main/web/src/components/timeline/preview/gpu/__tests__/compositor.test.ts), [`timeline/__tests__/TimelineRenderer.test.tsx`](https://github.com/nodetool-ai/nodetool/blob/main/web/src/components/timeline/__tests__/TimelineRenderer.test.tsx), and the preview tests.
...
**Acceptance:** The single-sample output matches reference frames for opaque, translucent, zero-alpha, chroma key, masks, and generated mattes. Motion blur remains unchanged. The per-pixel CPU scan leaves the hot path. Report standalone readback and end-to-end export timings, separately. Run [`packages/timeline/tests/render.parity.gpu.test.ts`](https://github.com/nodetool-ai/nodetool/blob/main/packages/timeline/tests/render.parity.gpu.test.ts), [`packages/video-nodes/tests/timeline-alpha-render.test.ts`](https://github.com/nodetool-ai/nodetool/blob/main/packages/video-nodes/tests/timeline-alpha-render.test.ts), and output-format checks.

### S4 — Two-frame headless pipeline

After S3, split render submission/readback completion from encoder write through a small ordered frame result interface. Allow at most two frames in flight and one ordered encoder consumer. In `rawFrames.ts`, make a pending encoder `write()` settle when FFmpeg fails or `abort()` destroys stdin, even if it was waiting for `drain`. Honor encoder backpressure; on abort or error, stop scheduling, release mapped buffers and sources, abort encoding, and surface the original failure. Do not decode two samples from one forward-only FFmpeg stream concurrently. Verify whether the shared compositor's ping-pong textures are safe while a prior readback is pending before allowing a second submit. If they are not, add the required target ownership inside `HeadlessFrameCompositor` rather than exposing GPU buffers to the caller.

**Acceptance:** Alternating-color frames prove order and data ownership at 1080p and 4K. Cancellation works during decode, GPU mapping, and blocked encoder writes. Tests force abort and FFmpeg failure while `write()` awaits `drain`, assert prompt settlement, and preserve the original error. Memory and queue depth stay bounded. Alpha, masks, changing scene content, and motion-blur output match the serial path. Measure total export throughput with real codec decode and encode as well as synthetic frame throughput; retain the pipeline only if the end-to-end result is non-regressing on the representative fixture.

### L1 and L2 — Preview cost controls

**L1:** Replace entry-count-only limits for sequence-sized text and shape bitmaps with eviction by estimated bytes. Count both `TextRasterizer.cache` and `TextRasterizer.activeBitmaps`, plus shape bitmaps. Use an explicit frame scope at preview and browser-export call sites: pin every bitmap returned during source resolution until that frame has consumed or uploaded it, then release pins. Close only unpinned evictions. Define an oversized-live-frame policy: the live frame may temporarily exceed the cache budget, report those bytes, and evict after frame release. Measure resident cache bytes and texture allocations in the L3 scenarios. S1 owns same-size GPU texture reuse in `preview/gpu/compositor.ts`; L1 verifies that changed bitmap content still uploads after S1 merges. A later cropped-raster change needs explicit offsets and visual parity for crop, placement, and source-pixel effects.
...
**Acceptance:** L1 demonstrates a byte cap between frames, closes evicted bitmaps only after frame consumption, and does not serve a stale bitmap or texture on a same-size content change. A preview and browser-export fixture with one live frame larger than the budget must render every layer correctly, then return below the cap after release. L2 demonstrates correct crop, placement, captions, masks, and paused-frame sharpness at every setting. Browser export and server export are unaffected. L3 reports backing and source texture sizes separately.

### S5–S8 — Decode and source-cost track

Start these only after L3 shows which cost dominates on the target machines.
...
**Acceptance:** Each task has a before and after latency distribution from L3, bounded memory and queues, frame-accurate fixtures, cancellation tests, and a working fallback. Keep a change only when the representative end-to-end scenario improves without a correctness regression.

## Integration order and decision gates

Merge tasks in order within each track below. Preview and export tasks with disjoint files may interleave after their listed dependencies are satisfied.
