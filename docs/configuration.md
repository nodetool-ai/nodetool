...
| `NODETOOL_TSC_HEAP_MB` | V8 heap (MiB) given to the `tsc` child processes `npm run build:packages` and `npm run typecheck` spawn | no | Build-time only, not read by the server. Default `8192`, passed as `--max-old-space-size`. Digits only — any other value falls back to the default. Ignored entirely when `NODE_OPTIONS` already carries a `--max-old-space-size`, so your own tuning is never overwritten. Raise it when a build or typecheck dies with `JavaScript heap out of memory`. Node's own default is derived from the machine (2 GiB on a 16 GiB host) and `web`'s program needs roughly 4.5 GiB, so `npm run typecheck` routes every `tsc` through `scripts/run-tsc.mjs` to apply this ceiling |
