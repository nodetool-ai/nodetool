...
| `NODETOOL_PUBLIC_URL` | HTTPS base URL reachable by browsers and hosted provider callbacks | no | Trailing slashes are stripped. Used for integration link URLs and for FAL callback URLs at `/api/providers/fal/webhook/:token`. FAL callbacks are included only when this is a valid `https` URL that FAL can reach. Unset, local FAL requests use queue polling. Integration links still fall back to the request's `Host` header, which may be useful when a bridge reaches the server at an address the user's browser cannot, such as `http://nodetool:7777` inside a compose network. See [telegram-bot-design.md](telegram-bot-design.md) §5 |
