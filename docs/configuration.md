...
| `NODETOOL_DISABLE_MCP_OAUTH` | Turn off the MCP OAuth 2.1 authorization-server surface | no | `1` only. Disabled: `/mcp` never sends a `WWW-Authenticate` challenge, and `/.well-known/oauth-*`, `/oauth/authorize`, `/oauth/token`, `/oauth/register`, and `/oauth/revoke` all 404. Pasting an `ntk_` token (**Settings → MCP → Connect an agent remotely**) stays the only way to connect. The flow this disables needs no enable flag of its own — it activates whenever `NODETOOL_ENABLE_MCP` and `NODETOOL_PUBLIC_URL` are both set, the same gate the mount itself uses. See [MCP OAuth design](mcp-oauth-design.md) |
