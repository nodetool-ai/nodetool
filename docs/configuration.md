...
| `DATABASE_POOL_MAX` | Maximum PostgreSQL connections one server process opens | no | Default 10. A Supabase session-mode pooler (port 5432 on `pooler.supabase.com`) holds one server connection per client and rejects new clients with `EMAXCONNSESSION` at its pool size, so this value times the number of server processes must stay below that size. A transaction-mode URL (port 6543) shares server connections, and the server turns off prepared statements for it |
| `DIRECT_URL` | Dedicated PostgreSQL URL used by migration commands and the release-phase migration runner | no | Resolution order is `--direct-url`, `--database-url`, `DIRECT_URL`, then `DATABASE_URL`; with none of them the command fails naming all four. On Supabase this is the project's **direct** connection URL, which is separate from the pooled `DATABASE_URL` used by the server. Set it on deployments so migrations do not exhaust the pooler's session limit. See [CLI › Database Migrations](cli.md#database-migrations) |
| `NODETOOL_STORAGE_AUTO_CLEANUP` | Expire database history on a schedule instead of when someone asks | no | `1` or `true`, trimmed and case-insensitive; unset, `0`, or anything else keeps the sweep manual. It only supplies the **default** for the per-user `storage.retention.automaticCleanup` setting, so a value saved from the UI wins over it. Turned on, the server sweeps at startup and every 6 hours, and each sweep does work only if the last one finished more than 24 hours ago: workflow autosaves past 10 per workflow or 7 days, manual versions past 90 days, terminal jobs past 30 days, run events past 30 days, and the `parameters`/`metadata`/`logs` columns of predictions past 400 days (prediction rows themselves are billing records and survive). Current workflows, assets, messages, memories, and authored documents are never candidates, and each window is overridable per user under `storage.retention.*`. The startup sweep runs for user `1` only, so on a multi-user deployment it covers that account and no other. Set it where a published retention promise has to hold without anyone pressing a button |
...
| `NODETOOL_MAX_UPLOAD_BYTES` | Largest payload a single storage upload may write | no | Default `1073741824` (1 GiB) for cloud and in-memory storage, `2147483648` (2 GiB) for local file storage reads and writes, including media staging. An explicit value applies to both paths. Same strict parsing as the frame cap |
...
| `NODETOOL_SHUTDOWN_GRACE_MS` | How long SIGTERM waits for aborted turns and runs to settle before the process exits | no | Default `240000`; anything unparseable or negative keeps it. Under Fly's 300 s cap between the signal and the kill. It bounds only the fallback path: a deploy drains a machine with SIGUSR2 first and waits on `/health` instead, which has no deadline. Turns and runs still executing when it expires are logged and dropped, so their transcripts stay incomplete. See [WebSocket API › Draining](websocket-api.md#draining) |
...
| `NODETOOL_TSC_VERSION` | Select the compiler used by the shared TypeScript launcher | no | `7` by default for the native TypeScript 7 CLI. Set to `6` for the TypeScript 6 JavaScript compiler when using a rollback or comparing compiler diagnostics. The value must be `6` or `7`; invalid values fail before compilation. Compiler-API consumers continue to import the pinned TypeScript 6 compatibility package regardless of this setting |
| `NODETOOL_TSC_HEAP_MB` | V8 heap (MiB) given to the TypeScript 6 JavaScript compiler child process | no | Build-time only, not read by the server. Default `8192`, passed as `--max-old-space-size` only when `NODETOOL_TSC_VERSION=6`. Digits only — any other value falls back to the default. Ignored entirely when `NODE_OPTIONS` already carries a `--max-old-space-size`. The TypeScript 7 native compiler does not receive this Node heap option |
