...
- **No `any`** in new code. `web/src`, `electron/src`, and `mobile/src` have reached zero `any` outside tests, and `@typescript-eslint/no-explicit-any` is now an **error** in all three so it stays that way. The one exemption is `PropertyProps`' default type argument in `web/src/components/node/PropertyInput.types.ts`, which the property-editor registry depends on. The ESLint override that allows `any` in `packages/*` (see `eslint.config.mjs`) is a transitional concession — new code must use `unknown` + narrowing, generics, or proper types.
...
| Type safety | `anti-slop/no-hand-written-any` + `no-implicit-return-type` in `npm run lint`; `anti-slop-ratchet.yaml` works the backlog |
