...
NodeTool ships eleven (`packages/sandbox-packs/`): `-dates` (date-fns) and
`-yaml` (js-yaml) run inside the guest; `-csv` (papaparse), `-html` (cheerio +
turndown), `-xml` (fast-xml-parser), `-xlsx` (exceljs), `-diff` (diff), `-zip`
(fflate), `-sqlite` (better-sqlite3), `-ocr` (tesseract.js) and `-tfjs`
(TensorFlow.js and its model zoo) run on the host behind a generated facade,
because they need Node builtins, a DOM or a native addon, or carry a limit the
guest could not enforce on itself (zip's 50 MB inflation cap), or hold state no
run can keep alive (the tfjs weights). Third-party packs install the same way.
See
...
- **`tools.<name>(args)`** — one wrapper per tool on the belt, generated by
  `TOOLS_PRELUDE` over a single `__callTool` bridge. A tool returning an
