...
- **`import { <name> } from "@nodetool-ai/sandbox-nodetool/<namespace>"`** —
  the belt, reached the way a library is. There is no `tools.<name>()` global:
  a name a capability module owns comes from that namespace, a client `ui_*`
  tool from `.../ui`, and anything the session added at its own call site from
  `.../session`. One import shape covers all three, and a body's imports say
  what it uses before it runs. A tool returning an
  ...
  `tools` still exists, as a thrower. Every property access reports the import
...
- **`nodetool.searchTools(query)`** — in-sandbox discovery for tools the prompt lists by
  name only, past the disclosure threshold. Each hit carries `name`,
  `signature`, `description`, and the `module` / `specifier` / `import` line
  that makes it callable, so discovery answers "where do I import this from"
  in the same round trip as "what are its arguments". Deferred tools stay
  callable; the split spends prompt tokens, not capability.
...
imported belt function is a tool the model could have called directly, and
per-step allowlists stay a privilege boundary. What is genuinely new is composition —
...
| `tools.<name> is gone` | The flat toolbelt was retired for imports. The message names the import that replaces the call, or points at `nodetool.searchTools(name)` when no module owns the name |
| `Could not find export '<name>' in module '…/<namespace>'` | The namespace is mounted but this host's belt has no such tool. `nodetool.searchTools(name)` lists what it does have; `ui_*` is browser-only and never on a Code node's belt |
