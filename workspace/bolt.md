...
## 2026-05-18 - NodeStore `getSelectedNodeIds` `O(N)` Selection Optimization
**Learning:** The `getSelectedNodeIds` function in the Zustand store currently iterates through the nodes array on every call to collect selected node IDs. Like `getSelectedNodes` and `getSelectedNodeCount`, since `getSelectedNodeIds` is an `O(N)` operation and can be called inside components or hooks reacting to node changes, evaluating it without a reference cache creates unnecessary processing and potential performance issues during high-frequency updates like dragging.
**Action:** Implement the same lightweight internal reference caching strategy for `getSelectedNodeIds` that is used for `getSelectedNodeCount` and `getSelectedNodes` by maintaining `lastNodesForSelectionIds` and `lastSelectedNodeIds` closures in the store initialization.
## 2026-05-18 - NodeStore `getSelection` `O(N+E)` Selection Optimization
**Learning:** The `getSelection` function in the Zustand store currently iterates through the nodes array to filter selected nodes, builds an intermediate record of IDs, and then iterates through the entire edges array to filter edges connecting those nodes. Since this is an `O(N + E)` operation and is heavily called without a reference cache, it creates unnecessary processing overhead, especially for large graphs during frequent state updates.
**Action:** Implement the same lightweight internal reference caching strategy for `getSelection` that is used for `getSelectedNodeCount`. The cache should check both `nodes` and `edges` reference equality (`nodes === lastNodesForGetSelection && edges === lastEdgesForGetSelection`) to return the cached `lastSelection` object.
## 2026-02-17 - Optimizing Log Subscriptions with Zustand
**Learning:** Zustand's default selector equality is strict equality (`===`). If a selector returns a new reference (like `array.filter()` does), the component will re-render on EVERY store update, even if the data content is identical. This is O(N) re-renders for N components.
**Action:** Use `useStoreWithEqualityFn` (from `zustand/traditional`) with `shallow` equality (from `zustand/shallow`) when selecting derived arrays or objects to prevent unnecessary re-renders.
