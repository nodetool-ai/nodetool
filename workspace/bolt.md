...
## 2024-03-08 - Zustand Equality functions for array return types
**Learning:** Returning arrays from custom Zustand selectors like `state.getSelectedNodes()` (which returns `get().nodes.filter((node) => node.selected)`) causes Zustand's default strict equality check (`===`) to fail on every single store update, even if the selected nodes remain completely identical.
**Action:** When a hook or selector relies on an array derived from `.filter()` or `.map()` that produces a newly-allocated array on each tick, supply a custom deep or shallow equality function (like `(a, b) => a.length === b.length && a.every((v, i) => v === b[i])`) directly in the `useNodes` call so that React components using this array do not suffer from continuous `O(N)` re-renders.
