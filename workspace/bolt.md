...
## 2025-02-18 - [Optimizing Array Lookups in Streaming Updates]
**Learning:** High-frequency event streaming that updates arrays (like tool calls or log steps) by continuously rebuilding state over a full array of messages creates an O(N²) bottleneck if `array.findIndex()` is used, because it always scans from the beginning. Since these updates almost exclusively apply to newly appended or recent items, scanning from the back is exponentially more efficient.
**Action:** When updating elements in arrays that receive sequential appends (particularly during streaming responses or state rebuilding), use `findLastIndex()` or an O(1) Map instead of `findIndex()`.
