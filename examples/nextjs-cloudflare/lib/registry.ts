 ...
 * native bindings that can't bundle for the Workers (V8 isolate) runtime.
 *
...
// `openai.text.WebSearch` calls OpenAI with the global `fetch`, so it bundles
// cleanly for workerd. We use the `@nodetool-ai/llm-nodes/openai` subpath — it
// avoids the native OS-keychain chain that the `agents` nodes pull in. Only
 ...
 * no `child_process`, no filesystem — safe for the Workers isolate.
 */
...
export interface RegistryOptions {
  /** Also register the LLM nodes (needs a provider key at run time). */
  includeLlm?: boolean;
}
