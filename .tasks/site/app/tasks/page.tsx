import { getAllPlans, getAllTasks, STATE_LABEL, TASK_STATES } from "@/lib/tasks";
import { TaskRow } from "@/components/task-row";
import { StateIcon } from "@/components/state-icon";

export default function TasksIndexPage() {
  const tasks = getAllTasks();
  const planTitles = new Map(getAllPlans().map((p) => [p.id, p.title] as const));
  ...
  return (
    <div className="space-y-8">
      <header>
        ...
        <p className="text-sm text-muted-foreground">{tasks.length} total &middot; grouped by state.</p>
      </header>
