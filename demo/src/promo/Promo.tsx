/**
 * The landing-page / social product video, assembled from five scenes over
 * 1350 frames (45s @ 30fps) — the master cut specified in
 * marketing/VIDEO_SCRIPT.md:
 *
 *   Hook      0–96      the finished trailer pulls back into the editor
 *   Canvas    88–508    Act 1: four takes render on the real graph canvas
 *   Timeline  500–1012  Act 2: the cut is built on the real timeline editor
 *   Cost      1012–1164 the honesty beat: your keys, provider prices
 *   Close     1164–1350 export → finale full-frame → brand card
 *
...
export const PROMO_DURATION_FRAMES = 1350;
export const PROMO_FPS = 30;

const CANVAS_FROM = 88;
const TIMELINE_FROM = 500;
const COST_FROM = 1012;
const CLOSE_FROM = 1164;
...
export const Promo: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: PROMO_BG }}>
      <Sequence from={0} durationInFrames={CANVAS_FROM + 8} name="Hook">
        <HookScene />
      </Sequence>
      ...
      {/* Music bed under the UI acts — the same score that sits on the
          timeline's audio track in Act 2. The hook and close carry the
          trailer's own sound instead. */}
      <Sequence from={CANVAS_FROM} durationInFrames={CLOSE_FROM - CANVAS_FROM}>
        <Audio
          ...
          volume={(f) =>
            interpolate(
              f,
              [0, 24, COST_FROM - CANVAS_FROM, CLOSE_FROM - CANVAS_FROM - 12, CLOSE_FROM - CANVAS_FROM],
              [0, 0.17, 0.17, 0.06, 0],
