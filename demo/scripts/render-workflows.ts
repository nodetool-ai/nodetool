/**
 * Batch-render the workflow-gallery videos (and still thumbnails).
 *
 *   npm run render:workflows            # all → MP4 + JPG
 *   npm run render:workflows -- --stills-only
 *   npm run render:workflows -- --only transcribe-audio
 *
 * Bundles the Remotion project once and renders every `Workflow-<slug>`
 * composition, writing docs/assets/workflows/<slug>.mp4 and <slug>.jpg (where the
 * docs Workflow Gallery pages embed them). Bundling once (rather than per
 * `remotion render` CLI call) makes the renders far cheaper. Set CHROMIUM_PATH to
...
const outDir = path.resolve(demoRoot, "../docs/assets/workflows");
...
const stillFrame = (entry: (typeof WORKFLOWS)[number]): number => {
...
async function main() {
  const entries = onlySlug
    ? WORKFLOWS.filter((e) => e.slug === onlySlug)
    : WORKFLOWS;
  if (entries.length === 0) throw new Error(`No workflow matched --only ${onlySlug}`);

  await mkdir(outDir, { recursive: true });

  console.log(`Bundling Remotion project (${entries.length} workflow(s))…`);
