...
- [ ] Step 10: LongLive.
  ...
  - Upstream output normalization landed: causal samplers can extract video tensors from raw outputs, dict wrappers (`video`/`videos`/`frames`), and object wrappers (`.video`/`.videos`/`.frames`), then select frames from batched, time-first, or single-frame `CHW`/`HWC` outputs before realtime conversion.
