...
- [ ] Step 10c: Model artifact and loader handling for realtime video backends.
  ...
  - Compatibility task: validate model-family compatibility before applying adapters. Check base size/family (`1.3B` vs `14B`), checkpoint shape/key expectations, LoRA target keys, VACE module expectations, precision format, and sampler assumptions before mutating a loaded model.
  - LoRA task: promote LoRA handling from ad hoc hooks to a shared adapter contract with provenance, strength/merge mode, compatible target reporting, and lazy optional dependencies. Keep LongLive's bundled LoRA path working; add Self-Forcing acceleration LoRA support only after a compatible 1.3B candidate is identified and tested.
  - Quantization task: split quantization into explicit strategies: pre-quantized artifacts (`fp8` safetensors, `gguf` text encoder) vs post-load transforms (for example torchao FP8 on denoiser). Each strategy must declare device requirements, CPU-offload compatibility, and install extras.
  - VACE/control task: keep VACE as a separate optional control-extension stage with its own manifest, load order, control inputs, and memory profile. Do not require VACE for the base `SelfForcing` node.
  - Memory task: add RTX 3060 12GB telemetry for artifact sizes, loaded/offloaded modules, free/reserved VRAM, dtype choices, cache size, and fallback decisions. Use this to decide whether GGUF text encoding, CPU text encoder offload, TAEHV/TinyVAE, FP8 denoiser, or lower frame/block settings are required.
  - Lifecycle task: unload/reload stale model instances when artifact, precision, VACE, LoRA, or sampler settings change, and make that behavior visible in loading events to avoid hidden OOMs across realtime sessions.
  - Smoke task: define two smoke tiers: canonical official smoke (`Wan-AI/Wan2.1-T2V-1.3B` + `gdhe17/Self-Forcing:checkpoints/self_forcing_dmd.pt` + official config) and RTX 3060 low-VRAM smoke (validated FP8/GGUF/VAE choices). Both must be opt-in and skipped by default.
