/**
 * Helper utilities for efficient waiting in Electron E2E tests
 *
 * Adapted from the web E2E helpers. The key difference is that
 * navigateToPage resolves relative URLs against the Electron window's
 * current origin (which may be file:// or http://localhost:...).
 */
...
export async function waitForPageReady(page: Page): Promise<void> {
  ...
  try {
    ...
    // If React hydration check fails, page might still be functional
  }
}
...
export async function navigateToPage(
  ...
  // If the URL is relative, resolve it against the current page origin
  let targetUrl = url;
  if (url.startsWith("/")) {
    const currentUrl = page.url();
