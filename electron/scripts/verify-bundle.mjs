...
// Backend bundle layout (manifests, examples, assets, webgpu dawn binaries)
// is verified by verify-backend-bundle.mjs, which bundle-backend.mjs runs
// automatically after staging — so it fires in every pipeline that stages
// the backend, not just the ones that happen to run this script afterwards.
