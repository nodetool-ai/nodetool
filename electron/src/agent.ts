...
import { query, tool, createSdkMcpServer, listSessions, getSessionMessages } from "@anthropic-ai/claude-agent-sdk";
...
class ClaudeAgentSession implements AgentQuerySession {
  ...
  async send(
    ...
    try {
      ...
      let mcpServers: Record<string, ReturnType<typeof createSdkMcpServer>> | Record<string, { type: "http"; url: string }> | undefined;
      ...
      if (this.useStandardTools) {
        ...
        if (this.mcpServerUrl) {
          mcpServers = { "nodetool-ui": { type: "http" as const, url: this.mcpServerUrl } };
        }
      }
