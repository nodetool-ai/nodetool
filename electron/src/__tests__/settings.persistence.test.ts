...
// ---------------------------------------------------------------------------
// getModelServiceStartupSettings — edge cases
// ---------------------------------------------------------------------------

describe("getModelServiceStartupSettings edge cases", () => {
  test("explicit false overrides llama_cpp default of true", async () => {
    const settingsDir = path.join(tempDir, ".config", "nodetool");
    fs.mkdirSync(settingsDir, { recursive: true });
    fs.writeFileSync(
      path.join(settingsDir, "settings.yaml"),
    ...
    const { getModelServiceStartupSettings } = await import("../settings");
    const result = getModelServiceStartupSettings();
    expect(result.startLlamaCppOnStartup).toBe(false);
  });

  test("non-boolean START_LLAMA_CPP_ON_STARTUP falls back to default", async () => {
    const settingsDir = path.join(tempDir, ".config", "nodetool");
    fs.mkdirSync(settingsDir, { recursive: true });
    fs.writeFileSync(
      path.join(settingsDir, "settings.yaml"),
      yaml.dump({
        MODEL_BACKEND: "llama_cpp",
        START_LLAMA_CPP_ON_STARTUP: "yes",
      }),
      "utf8"
    );
    ...
    const { getModelServiceStartupSettings } = await import("../settings");
    const result = getModelServiceStartupSettings();
    // "yes" is not a boolean, so it should fall back to the llama_cpp default (true)
    expect(result.startLlamaCppOnStartup).toBe(true);
  });

  test("uses passed-in settings object instead of reading from disk", async () => {
    const { getModelServiceStartupSettings } = await import("../settings");
    const result = getModelServiceStartupSettings({
      MODEL_BACKEND: "llama_cpp",
      START_LLAMA_CPP_ON_STARTUP: false,
  ...
  test("defaults to false when no MODEL_BACKEND and no explicit setting", async () => {
    const { getModelServiceStartupSettings } = await import("../settings");
    const result = getModelServiceStartupSettings({});
    expect(result.startLlamaCppOnStartup).toBe(false);
  });
...
// ---------------------------------------------------------------------------
// updateModelServiceStartupSettings — merging behavior
// ---------------------------------------------------------------------------

describe("updateModelServiceStartupSettings", () => {
  test("merges partial update with current settings", async () => {
    const settingsDir = path.join(tempDir, ".config", "nodetool");
    fs.mkdirSync(settingsDir, { recursive: true });
    fs.writeFileSync(
      path.join(settingsDir, "settings.yaml"),
      yaml.dump({
        MODEL_BACKEND: "llama_cpp",
        START_LLAMA_CPP_ON_STARTUP: true,
      }),
      "utf8"
    );

    const { updateModelServiceStartupSettings } = await import("../settings");
    const result = updateModelServiceStartupSettings({
    ...
    expect(ondisk.START_LLAMA_CPP_ON_STARTUP).toBe(false);
  });

  test("empty update preserves current settings", async () => {
    const { updateModelServiceStartupSettings, readSettings } = await import(
      "../settings"
    );

    // Start with defaults (no file, no backend)
    const result = updateModelServiceStartupSettings({});
    expect(result.startLlamaCppOnStartup).toBe(false);

    const settings = readSettings();
    expect(settings.START_LLAMA_CPP_ON_STARTUP).toBe(false);
  });
});
