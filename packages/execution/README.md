...
| `packages/agents/src/execute-agent-graph.ts:122` (`executeAgentGraph`) | the facade's, via `ExecutionSession.create({resolveNodeType})`; `applyRunPolicy` first stamps provider/model/prompt onto the planned `GraphData` | the facade's, from the passed `registry` plus `createGraphNodeTypeResolver(registry).resolveNodeType` | `params: opts.inputs ?? {}` | `session.cancel("cancelled")`, driven by `opts.signal` and by an early-`break` consumer | none | none | none | **Migrated.** `AgentWorkflowRunner` was flattened into `executeAgentGraph` (#4907), which drives `ExecutionSession` instead of constructing a `WorkflowRunner`. `applyRunPolicy` stays agent-owned — it is a graph transform, not execution wiring. Persistence stays absent: this streams into a chat turn, not a job |
