...
task breakdown this package implements (A2). Task A3 migrates the CLI, debug
harness, and headless job runner onto this facade; A5 migrates the WS runner
last. **This package does not migrate any call site** — every existing
construction site listed below is unchanged.
...
| `packages/websocket/src/http-api.ts:825` (async HTTP run) | `hydrateGraphNodeFlags` via a shared `getWorkflowRuntimeEnvironment(options)` helper (its own registry+bridge bootstrap, distinct from the CLI/debug/headless copy-paste *and* from the WS runner's injected callback — a third resolution pattern) | `runtime.resolveExecutor` (method on the shared runtime helper, itself presumably 3-branch internally — not duplicated inline here) | params only | none | none | `Job.create` before run; `markCompleted`/`markCancelled`/`markFailed` after, with a try/catch that fails the row instead of leaving it stuck `"running"` | none | **facade-owned**: hydration, the try/finally-safe persistence pattern (`onAccepted`/`onTerminal` hooks generalize the "never strand a running row" fix already present here). The `getWorkflowRuntimeEnvironment` bootstrap itself stays **out of scope** (it also builds the registry and ensures the Python bridge is connected — infra construction, not execution semantics) |
| `packages/websocket/src/mcp-server.ts:536` (MCP tool: run workflow) | `hydrateGraphNodeFlags` via `getRuntimeEnvironment(options)` (same third pattern as http-api.ts) | `runtime.resolveExecutor` | params only | none | none | `Job.create` + terminal status, same shape as http-api.ts | none | **facade-owned**, identical reasoning to the http-api.ts row — these two sites are near-duplicates of each other |
...
| `packages/websocket/e2e/src/browser-entry.ts:149` | same pattern as `run.ts` (it's the e2e harness's copy of the browser runner) | `opts.registry.resolve` | params only | `AbortSignal` | none | none | none | **out of scope**: test harness, not a product surface |

### Notes on what didn't make the table

- **`packages/websocket/src/mcp-server.ts`** and **`http-api.ts`** share a
  helper (`getRuntimeEnvironment`/`getWorkflowRuntimeEnvironment`) that itself
  wraps registry + Python-bridge bootstrap and exposes `resolveExecutor` as a
  method. That helper is a *third* copy of "registry → Python bridge →
  throw" logic, independent of the one duplicated verbatim in
  `cli/nodetool.ts`, `debug/server-runner.ts`, and `headless-job-runner.ts`.
  This package's `createExecutorResolver` (`src/executor-resolver.ts`)
  generalizes the copy-pasted version; migrating `http-api.ts`/`mcp-server.ts`
  onto it is A3+ work, not done here.
