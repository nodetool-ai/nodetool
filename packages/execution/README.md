...
job runner, HTTP API, MCP server, the WebSocket runner). Task A3 migrated the CLI, debug
harness, and headless job runner onto this facade; A5 migrated the WS runner
(then `unified-websocket-runner.ts`, since renamed to
`packages/websocket/src/websocket-client-session.ts`) last, once the reliability
harness could watch it for drift. The wiring-inventory table below is a
historical snapshot from before those migrations, and still uses the old file
name and its line numbers — each migrated row is annotated with what actually
...
| `packages/agents/src/execute-agent-graph.ts:122` (`executeAgentGraph`) | — | — | — | — | — | — | — | **Gone.** The retired `Agent` graph branch was its only caller, so it was deleted with it; `applyRunPolicy`, a graph transform rather than execution wiring, survives in `packages/agents/src/run-policy.ts` for the `graph-e2e` eval |
...
- **`nodeTimeoutMs`** has no kernel-side hook today (only the run-level `AbortSignal` that
