 ...
 * because choosing a search provider is the host's job, not the model's.
 */
...
/** Google organic results for a keyword. */
export async function dataForSeoSearch(
  context: ProcessingContext,
  params: { keyword?: unknown; num_results?: unknown }
...
/** Google News articles for a keyword. */
export async function dataForSeoNews(
  context: ProcessingContext,
  params: { keyword?: unknown; num_results?: unknown }
): Promise<DataForSEOResult> {
  const keyword = params.keyword as string | undefined;
  ...
  const newsResults = items
    .filter((item) => item.type === "news_search" || item.type === "top_stories")
    .map((item) => {
...
/** Google Images results for a keyword, or a reverse search for an image URL. */
export async function dataForSeoImages(
  context: ProcessingContext,
  params: { keyword?: unknown; image_url?: unknown; num_results?: unknown }
): Promise<DataForSEOResult> {
  const keyword = params.keyword as string | undefined;
