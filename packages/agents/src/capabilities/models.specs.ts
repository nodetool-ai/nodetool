...
export const findModelSpec: CapabilitySpec = {
  name: "find_model",
  description:
    "Find a real {provider, model_id} for a generic AI node by capability. Omit `query` and the first result is the best model for the job: image, video, speech and music candidates are ordered by their quality leaderboard for the task. Pass `query` only when the user named a model ('flux schnell'), or `provider_hint` / `model_hint` / `prefer_local` when they stated a preference — those outrank the leaderboard. Results are one row per model, not per route: a model several providers serve appears once, with the others under `alternate_routes`. The answer's top-level `ref` is the best model's typed value — assign it to a node's model property verbatim. Each result carries its own `ref` too, and, for ranked models, `canonical`, `ranked_task`, `rank`/`of`, and `alternate_routes` (other providers serving the same model). A result carrying `prompting_skill` names a shipped guide for that model line: read it with `load_skill` before you write the prompt. Call this before adding any generic AI node (TextToImage, TextToVideo, TextToSpeech, etc.) — never guess a model id.",
