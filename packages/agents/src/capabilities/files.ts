 ...
 * run. The classes survive as thin `CapabilityTool` subclasses with unchanged
 * zero-arg constructors, so `BUILTIN_TOOL_CLASSES` and `resolveTool(name)` see
...
const LIST_DIRECTORY_SCHEMA: JsonSchema = {
  type: "object",
  properties: {
    path: {
      type: "string",
...
const TODO_WRITE_SCHEMA: JsonSchema = {
  type: "object",
  properties: {
    todos: {
      type: "array",
