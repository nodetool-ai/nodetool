...
describe("@nodetool-ai/sandbox-apify", () => {
  it("builds the three requests of a run, tilde-escaping the actor id", async () => {
    const result = await run(
      `import { startRun, runStatus, datasetItems } from "@nodetool-ai/sandbox-apify";
       return {
         start: await startRun({ token: "t", actor: "apify/web-scraper", input: { a: 1 } }),
         poll: await runStatus({ token: "t", runId: "RUN1" }),
         items: await datasetItems({ token: "t", datasetId: "DS1", limit: 50 })
       };`,
      "apify"
    );
    expect(result.error).toBeUndefined();
    const { start, poll, items } = result.result as Record<
      string,
      { url: string; method: string; headers: Record<string, string>; body?: string }
    >;
    expect(start.url).toBe("https://api.apify.com/v2/acts/apify~web-scraper/runs");
    expect(start.method).toBe("POST");
    expect(start.body).toBe('{"a":1}');
    expect(start.headers.Authorization).toBe("Bearer t");
    expect(poll.url).toBe("https://api.apify.com/v2/actor-runs/RUN1");
    expect(items.url).toContain("/v2/datasets/DS1/items");
    expect(items.url).toContain("limit=50");
  });

  it("names the missing argument instead of building a broken URL", async () => {
    const result = await run(
      `import { startRun } from "@nodetool-ai/sandbox-apify";
       try { await startRun({ token: "t" }); return { threw: false }; }
       catch (e) { return { threw: true, message: String(e.message) }; }`,
      "apify"
    );
    expect(result.result).toMatchObject({
      threw: true,
      message: "apify.startRun: actor is required"
    });
  });
});
