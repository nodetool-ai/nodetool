...
describe("crypto", () => {
  ...
  describe("encryptFernet/decryptFernet", () => {
    it("should round-trip encrypt and decrypt", () => {
      const masterKey = generateMasterKey();
      const userId = "user-1";
      const plaintext = "my-fernet-secret-12345";
    ...
    it("should produce different tokens for same plaintext (due to random IV)", () => {
      const masterKey = generateMasterKey();
      const userId = "user-1";
      const plaintext = "same-secret";
    ...
    it("should produce different tokens for different users", () => {
      ...
      const token1 = encryptFernet(masterKey, "user-1", plaintext);
      const token2 = encryptFernet(masterKey, "user-2", plaintext);
    ...
    it("should reject tokens that are too short", () => {
      ...
      expect(() => decryptFernet(masterKey, userId, shortToken)).toThrow(
        "Invalid Fernet token: too short"
      );
    });

    it("should reject tokens with invalid version byte", () => {
      const masterKey = generateMasterKey();
      const userId = "user-1";
      // Create a valid-length token with wrong version byte (0x00 instead of 0x80)
      const fakeToken = Buffer.alloc(1 + 8 + 16 + 16 + 32, 0);
      const b64 = fakeToken.toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");

      expect(() => decryptFernet(masterKey, userId, b64)).toThrow(
        "Invalid Fernet token version"
      );
    });

    it("should reject tokens with tampered HMAC", () => {
      const masterKey = generateMasterKey();
      const userId = "user-1";
      const token = encryptFernet(masterKey, userId, "secret");

      // Decode and tamper with HMAC (last 32 bytes)
      const decoded = Buffer.from(
        token.replace(/-/g, "+").replace(/_/g, "/") + "==".slice((token.length % 4) || 4),
        "base64"
      );
      // Flip a bit in the HMAC
      decoded[decoded.length - 1] ^= 0x01;
      const tampered = decoded.toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");

      expect(() => decryptFernet(masterKey, userId, tampered)).toThrow(
        "Invalid Fernet token: HMAC mismatch"
      );
    });
    ...
    it("should produce base64url-encoded tokens without padding", () => {
      const masterKey = generateMasterKey();
      const userId = "user-1";
    ...
    it("token should start with version byte 0x80", () => {
      const masterKey = generateMasterKey();
      const userId = "user-1";
      const token = encryptFernet(masterKey, userId, "test");
      ...
      const decoded = Buffer.from(b64 + "==".slice((b64.length % 4) || 4), "base64");
...
describe("master-key", () => {
  ...
  describe("setMasterKeyPersistent", () => {
    it("should throw when keytar is not available", async () => {
      // In test environment, keytar is not available (native module)
      await expect(
        setMasterKeyPersistent("test-key")
  ...
  describe("deleteMasterKey", () => {
    it("should return false when keytar is not available", async () => {
      // In test environment, keytar is not available
