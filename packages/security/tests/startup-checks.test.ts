...
describe("T-SEC-8: runStartupChecks", () => {
  ...
  it("auto-generates master key when no source is configured", async () => {
    // No env var, no keytar, no AWS — initMasterKey will auto-generate
    const result = await runStartupChecks();
    expect(result.errors).toEqual([]);
  });

  it("uses env var master key via initMasterKey", async () => {
    process.env["SECRETS_MASTER_KEY"] = "env-startup-key";
    const result = await runStartupChecks();
    expect(result.errors).toEqual([]);
  });
});
