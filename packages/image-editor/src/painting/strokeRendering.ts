 ...
 *
 * Host-neutral: the only canvas it allocates goes through `surface.ts`, so the
 * same engine runs in the browser and headlessly in Node.
 */
...
import type { PaintContext2D, PaintSurface } from "./surface.js";
import { createPaintSurface } from "./surface.js";
...
export function drawBrushStroke(
  ...
  pressure: number | undefined,
  dirtyRect: DirtyRectTracker,
  brushStampCache: Map<string, PaintSurface>,
  stampState?: StrokeStampState
): void {
  ...
  const createBrushStamp = (
    ...
    const stamp = createPaintSurface(logicalD, logicalD);
    ...
    const drawAtLogicalScale = (targetCtx: PaintContext2D) => {
      targetCtx.save();
    ...
    const hi = createPaintSurface(hiPx, hiPx);
