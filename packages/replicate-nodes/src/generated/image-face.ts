...
replicate, ai`;
  ...
  @prop({ type: "float", default: 0.8, description: "Strength of depth controlnet. The bigger this is, the more controlnet affects the output.", min: 0, max: 1 })
  ...
  @prop({ type: "float", default: 0.65, description: "How much of the original image to keep. 1 is the complete destruction of the original image, 0 is the original image", min: 0, max: 1 })
  declare denoising_strength: any;

  @prop({ type: "image", default: "", description: "An image of a person to be converted" })
  declare image: any;

  @prop({ type: "float", default: 1, description: "How strong the InstantID will be.", min: 0, max: 1 })
  declare instant_id_strength: any;

  @prop({ type: "float", default: 1, description: "How strong the LoRA will be", min: 0, max: 1 })
  ...
  @prop({ type: "float", default: 4.5, description: "Strength of the prompt. This is the CFG scale, higher numbers lead to stronger prompt, lower numbers will keep more of a likeness to the original.", min: 0, max: 20 })
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    const res = await replicateSubmit(apiKey, "fofr/face-to-many:a07f252abbbd832009640b27f063ea52d87d7a23a185ca165bec23b5adc8deaf", args);
...
replicate, ai`;
  ...
  @prop({ type: "float", default: 1, description: "How much of the original image of the person to keep. 1 is the complete destruction of the original image, 0 is the original image", min: 0, max: 1 })
  declare denoising_strength: any;
  ...
  @prop({ type: "float", default: 0.3, description: "How much noise to add to the style image before processing. An alternative way of controlling stength.", min: 0, max: 1 })
  declare image_to_become_noise: any;

  @prop({ type: "float", default: 0.75, description: "How strong the style will be applied", min: 0, max: 1 })
  ...
  @prop({ type: "int", default: 2, description: "Number of images to generate", min: 1, max: 10 })
  declare number_of_images: any;

  @prop({ type: "str", default: "a person" })
  declare prompt: any;

  @prop({ type: "float", default: 2, description: "Strength of the prompt. This is the CFG scale, higher numbers lead to stronger prompt, lower numbers will keep more of a likeness to the original.", min: 0, max: 3 })
  declare prompt_strength: any;
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    const res = await replicateSubmit(apiKey, "fofr/become-image:8d0b076a2aff3904dfcec3253c778e0310a68f78483c4699c7fd800f3051d2b3", args);
    return { output: outputToImageRef(res.output) };
  }
}
...
replicate, ai`;
  ...
  @prop({ type: "float", default: 5, description: "Guidance scale. A guidance scale of 1 corresponds to doing no classifier free guidance.", min: 1, max: 10 })
  ...
  @prop({ type: "int", default: 1, description: "Number of output images", min: 1, max: 4 })
  declare num_outputs: any;

  @prop({ type: "int", default: 20, description: "Number of sample steps", min: 1, max: 100 })
  ...
  @prop({ type: "int", default: -1, description: "Seed. Leave blank to use a random number", min: 0, max: 2147483647 })
  declare seed: any;
  ...
  @prop({ type: "float", default: 20, description: "Style strength (%)", min: 15, max: 50 })
...
replicate, ai`;
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    const res = await replicateSubmit(apiKey, "tencentarc/photomaker-style:467d062309da518648ba89d226490e02b8ed09b5abc15026e54e31c5a8cd0769", args);
    return { output: outputToImageRef(res.output) };
  }
}
...
replicate, ai`;
  ...
  @prop({ type: "float", default: 0.5, description: "How much noise is added to the IP adapter input", min: 0, max: 1 })
  declare ip_adapter_noise: any;

  @prop({ type: "float", default: 0.2, description: "How much the IP adapter will influence the image", min: 0, max: 1 })
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    const res = await replicateSubmit(apiKey, "fofr/face-to-sticker:764d4827ea159608a07cdde8ddf1c6000019627515eb02b6b449695fd547e5ef", args);
    return { output: outputToImageRef(res.output) };
  }
}
...
replicate, ai`;
  ...
  @prop({ type: "float", default: 0.3, description: "Canny ControlNet strength, effective only if 'enable_canny_controlnet' is true", min: 0, max: 1 })
  declare canny_strength: any;

  @prop({ type: "float", default: 0.8, description: "Scale for IdentityNet strength (for fidelity)", min: 0, max: 1.5 })
  declare controlnet_conditioning_scale: any;

  @prop({ type: "float", default: 0.5, description: "Depth ControlNet strength, effective only if 'enable_depth_controlnet' is true", min: 0, max: 1 })
  ...
  @prop({ type: "int", default: 640, description: "Height of the input image for face detection", min: 640, max: 4096 })
  declare face_detection_input_height: any;

  @prop({ type: "int", default: 640, description: "Width of the input image for face detection", min: 640, max: 4096 })
  ...
  @prop({ type: "float", default: 0.8, description: "Scale for image adapter strength (for detail)", min: 0, max: 1.5 })
  declare ip_adapter_scale: any;

  @prop({ type: "float", default: 1.5, description: "Only used when 'enable_lcm' is set to True, Scale for classifier-free guidance when using LCM", min: 1, max: 20 })
  declare lcm_guidance_scale: any;

  @prop({ type: "int", default: 5, description: "Only used when 'enable_lcm' is set to True, Number of denoising steps when using LCM", min: 1, max: 10 })
  ...
  @prop({ type: "int", default: 1, description: "Number of images to output", min: 1, max: 8 })
  declare num_outputs: any;
  ...
  @prop({ type: "float", default: 0.4, description: "Openpose ControlNet strength, effective only if 'enable_pose_controlnet' is true", min: 0, max: 1 })
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    const res = await replicateSubmit(apiKey, "zsxkib/instant-id:2e4785a4d80dadf580077b2244c8d7c05d8e3faac04a04c02d8e099dd2876789", args);
    return { output: outputToImageRef(res.output) };
  }
}
...
replicate, ai`;
  ...
  @prop({ type: "float", default: 0.8, description: "Scale for ControlNet conditioning", min: 0, max: 1 })
  declare controlnet_conditioning_scale: any;

  @prop({ type: "float", default: 5, description: "Scale for classifier-free guidance", min: 1, max: 50 })
  declare guidance_scale: any;

  @prop({ type: "int", default: 640, description: "Height of output image", min: 512, max: 2048 })
  declare height: any;

  @prop({ type: "image", default: "", description: "Input image" })
  declare image: any;

  @prop({ type: "float", default: 0.8, description: "Scale for IP adapter", min: 0, max: 1 })
  declare ip_adapter_scale: any;
  ...
  @prop({ type: "int", default: 640, description: "Width of output image", min: 512, max: 2048 })
  declare width: any;
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    const res = await replicateSubmit(apiKey, "grandlineai/instant-id-photorealistic:03914a0c3326bf44383d0cd84b06822618af879229ce5d1d53bef38d93b68279", args);
    return { output: outputToImageRef(res.output) };
  }
}
...
replicate, ai`;
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    const res = await replicateSubmit(apiKey, "grandlineai/instant-id-artistic:9cad10c7870bac9d6b587f406aef28208f964454abff5c4152f7dec9b0212a9a", args);
    return { output: outputToImageRef(res.output) };
  }
}
