...
replicate, ai`;
  ...
  @prop({ type: "int", default: 32, description: "Batch size to use when processing text data.", min: 0 })
  declare batch_size: any;
...
replicate, ai`;
  ...
  @prop({ type: "int", default: 64, description: "Matryoshka dimension - output embedding dimension (64-1024)", min: 64, max: 1024 })
...
replicate, ai`;
  ...
  @prop({ type: "float", default: 200, description: "You probably don't need to worry about this parameter if you're just getting the embeddings for a handful of queries. This parameter sets the maximumum number of kibiTokens (1 kibiToken = 1024 tokens) to try to stuff into a batch (to avoid out of memory errors but maximize throughput). If the total number of tokens across the flattened list of requested embeddings exceed this value, the list will be split internally and run across multiple forward passes. This will not affect the shape of your output, just the time it takes to run.", min: 0.5 })
