...
replicate, ai`;
  ...
  @prop({ type: "float", default: 0.15, description: "The bigger this number is, the more ControlNet interferes", min: 0, max: 2 })
  declare condition_canny_scale: any;

  @prop({ type: "float", default: 0.35, description: "The bigger this number is, the more ControlNet interferes", min: 0, max: 2 })
  declare condition_depth_scale: any;

  @prop({ type: "float", default: 4, description: "Scale for classifier-free guidance", min: 1, max: 50 })
  ...
  @prop({ type: "float", default: 0.3, description: "IP Adapter strength.", min: 0, max: 1 })
  declare ip_scale: any;

  @prop({ type: "float", default: 0.9, description: "LoRA additive scale. Only applicable on trained models.", min: 0, max: 1 })
  ...
  @prop({ type: "float", default: 1, description: "If you want the image to have a solid margin. Scale of the solid margin. 1.0 means no resizing.", min: 1, max: 10 })
  ...
  @prop({ type: "float", default: 0.9, description: "When img2img is active, the denoising strength. 1 means total destruction of the input image.", min: 0, max: 1 })
...
replicate, ai`;
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    const canvasSize = this.canvas_size ?? [];
    const contentModeration = Boolean(this.content_moderation ?? false);
    const imageUrl = String(this.image_url ?? "");
    const negativePrompt = String(this.negative_prompt ?? "");
    const originalImageLocation = this.original_image_location ?? [];
    const originalImageSize = this.original_image_size ?? [];
