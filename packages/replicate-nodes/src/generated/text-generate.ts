...
replicate, ai`;
  ...
  @prop({ type: "float", default: 1, description: "A parameter that controls how long the outputs are. If < 1, the model will tend to generate shorter outputs, and > 1 will tend to generate longer outputs.", min: 0, max: 5 })
  declare length_penalty: any;

  @prop({ type: "bool", default: false })
  declare log_performance_metrics: any;

  @prop({ type: "int", default: 0, description: "This parameter has been renamed to max_tokens. max_new_tokens only exists for backwards compatibility purposes. We recommend you use max_tokens instead. Both may not be specified.", min: 1 })
  declare max_new_tokens: any;

  @prop({ type: "int", default: 512, description: "Maximum number of tokens to generate. A word is generally 2-3 tokens.", min: 1 })
  declare max_tokens: any;

  @prop({ type: "int", default: 0, description: "This parameter has been renamed to min_tokens. min_new_tokens only exists for backwards compatibility purposes. We recommend you use min_tokens instead. Both may not be specified.", min: -1 })
  declare min_new_tokens: any;

  @prop({ type: "int", default: 0, description: "Minimum number of tokens to generate. To disable, set to -1. A word is generally 2-3 tokens.", min: -1 })
  ...
  @prop({ type: "float", default: 0.7, description: "Adjusts randomness of outputs, greater than 1 is random and 0 is deterministic, 0.75 is a good starting value.", min: 0, max: 5 })
  declare temperature: any;

  @prop({ type: "int", default: 0, description: "When decoding text, samples from the top k most likely tokens; lower to ignore less likely tokens.", min: -1 })
  declare top_k: any;

  @prop({ type: "float", default: 0.95, description: "When decoding text, samples from the top p percentage of most likely tokens; lower to ignore less likely tokens.", min: 0, max: 1 })
  declare top_p: any;
...
replicate, ai`;
  ...
  @prop({ type: "float", default: 0.5, description: "Maximum image resolution in megapixels. Scales down image before sending it to Claude, to save time and money.", min: 0.001, max: 2 })
  declare max_image_resolution: any;

  @prop({ type: "int", default: 8192, description: "Maximum number of output tokens", min: 1024, max: 64000 })
  declare max_tokens: any;
  ...
  @prop({ type: "int", default: 1024, description: "Maximum number of tokens to use for extended thinking when enabled (only supported for Sonnet 3.7 and Sonnet 4)", min: 1024, max: 64000 })
...
replicate, ai`;
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    const inputItemList = this.input_item_list ?? [];
    const instructions = String(this.instructions ?? "");
    const jsonSchema = this.json_schema ?? "";
    ...
    const simpleSchema = this.simple_schema ?? [];
...
replicate, ai`;
  ...
  @prop({ type: "float", default: 0, description: "Frequency penalty parameter - positive values penalize the repetition of tokens.", min: -2, max: 2 })
  declare frequency_penalty: any;
  ...
  @prop({ type: "float", default: 0, description: "Presence penalty parameter - positive values penalize new tokens based on whether they appear in the text so far, increasing the model's likelihood to talk about new topics.", min: -2, max: 2 })
  declare presence_penalty: any;
  ...
  @prop({ type: "float", default: 1, description: "Sampling temperature between 0 and 2", min: 0, max: 2 })
  declare temperature: any;

  @prop({ type: "float", default: 1, description: "Nucleus sampling parameter - the model considers the results of the tokens with top_p probability mass. (0.1 means only the tokens comprising the top 10% probability mass are considered.)", min: 0, max: 1 })
  declare top_p: any;
...
replicate, ai`;
  ...
  @prop({ type: "int", default: 65535, description: "Maximum number of tokens to generate", min: 1, max: 65535 })
  declare max_output_tokens: any;
  ...
  @prop({ type: "float", default: 0.95, description: "Nucleus sampling parameter - the model considers the results of the tokens with top_p probability mass", min: 0, max: 1 })
  declare top_p: any;
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    const videosRefs = Array.isArray(this.videos) ? this.videos : [];
    const videosUrls: string[] = [];
    for (const ref of videosRefs) {
      if (isRefSet(ref)) {
        const url = await assetToUrl(ref as Record<string, unknown>, apiKey);
        if (url) videosUrls.push(url);
      }
    }
    if (videosUrls.length) args["videos"] = videosUrls;
    removeNulls(args);
...
replicate, ai`;
  ...
  @prop({ type: "int", default: 0, description: "Thinking budget for reasoning (0 to disable thinking, higher values allow more reasoning)", min: 0, max: 24576 })
...
replicate, ai`;
  ...
  @prop({ type: "int", default: 8192, description: "Maximum number of output tokens", min: 1024, max: 128000 })
  declare max_tokens: any;
...
replicate, ai`;
  ...
  @prop({ type: "float", default: 0, description: "Frequency penalty", min: -2, max: 2 })
  declare frequency_penalty: any;

  @prop({ type: "int", default: 1024, description: "The maximum number of tokens the model should generate as output.", min: 1, max: 16384 })
  declare max_tokens: any;

  @prop({ type: "float", default: 0, description: "Presence penalty", min: -2, max: 2 })
  declare presence_penalty: any;

  @prop({ type: "str", default: "", description: "Prompt" })
  declare prompt: any;

  @prop({ type: "float", default: 0.1, description: "The value used to modulate the next token probabilities.", min: 0, max: 2 })
  declare temperature: any;

  @prop({ type: "float", default: 1, description: "Top-p (nucleus) sampling", min: 0, max: 1 })
  declare top_p: any;
...
replicate, ai`;
  ...
  @prop({ type: "int", default: 1024, description: "Maximum number of tokens to generate.", min: 1, max: 131072 })
  declare max_tokens: any;
  ...
  @prop({ type: "float", default: 0.1, description: "Sampling temperature.", min: 0, max: 2 })
  declare temperature: any;
