 ...
 * `packages/automation-nodes/src/lib/browser-upload.ts`. Keep the two in sync.
 */
 ...
 * and hand the path to Chrome via `DOM.setFileInputFiles`. The file is kept
 * until process exit (Chrome reads it lazily). Throws if the path isn't
 * reachable by this Chrome.
 */
