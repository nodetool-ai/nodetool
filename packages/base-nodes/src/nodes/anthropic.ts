...
export class ClaudeAgentNode extends BaseNode {
  ...
  // TODO: process() awaits the full SDK response and only returns `text` —
  // `chunk` is never emitted. To actually stream, convert to `genProcess`
  // and yield each SDK message as a `chunk`, then flip `chunk` to
  // `iteration`. Keeping `single` so metadata reflects current behavior.
