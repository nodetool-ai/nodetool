...
function padArrays(a: NdArray, b: NdArray): [NdArray, NdArray] {
  ...
  const padA =
    a.data.length < maxLen
      ? {
          data: [...a.data, ...new Array(maxLen - a.data.length).fill(0)],
          shape: [maxLen]
        }
      : a;
  const padB =
    b.data.length < maxLen
      ? {
          data: [...b.data, ...new Array(maxLen - b.data.length).fill(0)],
          shape: [maxLen]
        }
      : b;
  return [padA, padB];
...
function elementwiseBinary(
  ...
  if (pa.data.length === 1) {
    return {
      data: pb.data.map((v) => fn(pa.data[0], v)),
      shape: [...pb.shape]
    };
  }
  if (pb.data.length === 1) {
    return {
      data: pa.data.map((v) => fn(v, pb.data[0])),
      shape: [...pa.shape]
    };
  }
  return {
  ...
  for (let o = 0; o < outerLen; o++) {
    for (let inner = 0; inner < innerLen; inner++) {
      ...
      for (let a = 0; a < axisLen; a++) {
        values.push(
          arr.data[o * axisLen * axisStride + a * axisStride + inner]
        );
      }
  ...
  private takeAlongAxis(
    ...
    for (let o = 0; o < outerLen; o++) {
      for (const selIdx of indices) {
        for (let inner = 0; inner < innerLen; inner++) {
          newData[idx++] =
            arr.data[o * axisLen * innerLen + selIdx * innerLen + inner];
        }
      }
    }
    ...
    for (const size of splitSizes) {
      ...
      for (let o = 0; o < outerLen; o++) {
        for (let s = 0; s < size; s++) {
          for (let inner = 0; inner < innerLen; inner++) {
            newData.push(
              arr.data[o * axisLen * innerLen + (offset + s) * innerLen + inner]
            );
          }
        }
      }
...
export class Reshape4DNode extends BaseNode {
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    return {
      output: { data: [...arr.data], shape: [rows, cols, depths, channels] }
    };
  }
}
...
export class ConvertToImageNode extends BaseNode {
  ...
  @prop({
    ...
    title: "Values",
    description:
      "The input array to convert to an image. Should have either 1, 3, or 4 channels."
  })
  declare values: any;
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    if (arr.data.length === 0)
      throw new Error("The input array is not connected.");

    const ndim = arr.shape.length;
    if (ndim !== 2 && ndim !== 3)
      throw new Error(
        "The array should have 2 or 3 dimensions (HxW or HxWxC)."
      );
...
export class ConvertToArrayNumpyNode extends BaseNode {
  ...
  @prop({
    ...
    title: "Image",
    description:
      "The input image to convert to a tensor. The image should have either 1 (grayscale), 3 (RGB), or 4 (RGBA) channels."
  })
  declare image: any;
