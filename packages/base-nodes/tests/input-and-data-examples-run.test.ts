...
describe("dataframe_reshape_cli", () => {
  it("pivots, iterates rows and repeats a value", async () => {
    const out = await run("dataframe_reshape_cli.json");

    // Pivot turns the long form into a cross-tab: one row per region, one
    // column per quarter.
    expect(out["pivoted"]).toEqual([
      {
        rows: [
          { region: "north", Q1: 10, Q2: 15 },
          { region: "south", Q1: 7, Q2: 3 }
        ]
      }
    ]);

    // ForEachRow streams every row, so it needs a Collect to be read as a list
    // — wired straight to an Output it would report only the last row.
    expect(out["rows"]).toEqual([
      [
        { region: "north", quarter: "Q1", amount: 10 },
        { region: "north", quarter: "Q2", amount: 15 },
        { region: "south", quarter: "Q1", amount: 7 },
        { region: "south", quarter: "Q2", amount: 3 }
      ]
    ]);

    // Schema is a declaration, not a transform: it emits the record type as
    // given.
    ...
    expect(out["repeated"]).toEqual([["tick", "tick", "tick"]]);
  });
});
