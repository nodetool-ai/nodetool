 ...
 *
 * This file lives in `@nodetool-ai/timeline` rather than beside the shared
 * field bags in `@nodetool-ai/protocol/api-schemas/timeline-tool-params.js`
 * because `ui_timeline_animate_clip` names {@link STAGGER_UNITS} and
 * {@link ANIMATED_PROPERTIES}, and protocol is the base package that this one
...
import { ANIMATED_PROPERTIES, STAGGER_UNITS } from "./animation/types.js";
...
export type TimelineAnimationInput = z.infer<typeof animationInput>;
...
export const TIMELINE_TOOL_CONTRACTS = {
  ...
  ui_timeline_animate_clip: {
    ...
    shape: {
      ...
      animations: z.array(animationInput).min(1)
    }
  },
...
export type TimelineToolName = keyof typeof TIMELINE_TOOL_CONTRACTS;
...
export type TimelineToolArgs<K extends TimelineToolName> = UiToolArgs<
  (typeof TIMELINE_TOOL_CONTRACTS)[K]
>;
