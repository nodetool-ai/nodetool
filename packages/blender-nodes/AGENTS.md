...
Substrate for `nodetool.blender.*` nodes (Stages 1–2): binary discovery,
...
and `RenderAnimation` nodes over `blender_ops/` (`render_image`,
`render_passes`, `render_animation` plus shared `common`, `depth`, `exr`).
