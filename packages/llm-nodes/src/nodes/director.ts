...
// The screenplay authoring rules — system prompt, schema, prompt shaping,
// parse, fallback — live in `@nodetool-ai/protocol` so this node and the
// storyboard's own direct `generate_text` request author the same way.
// Re-exported here because this module has been their import site.
export {
  buildDirectorPrompt,
  buildScreenplaySchema,
  fallbackScreenplay,
