...
describe("configure", () => {
  ...
  describe("configureSSH", () => {
    it("should return an ssh deployment with correct type", () => {
      const result = configureSSH("test", {
        host: "remote.com",
        sshUser: "deploy",
      });
    ...
    it("should set host and SSH user", () => {
      const result = configureSSH("test", {
        host: "10.0.0.5",
        sshUser: "admin",
      });
    ...
    it("should generate service name from port", () => {
      const result = configureSSH("test", {
        host: "remote.com",
        sshUser: "user",
        port: 7000,
      });
      expect(result.service_name).toBe("nodetool-7000");
    });
    ...
    it("should default SSH key path", () => {
      const result = configureSSH("test", {
        host: "remote.com",
        sshUser: "user",
      });
    ...
    it("should use custom SSH key path", () => {
      const result = configureSSH("test", {
        host: "remote.com",
        sshUser: "user",
        sshKeyPath: "/keys/my_key",
      });
      expect(result.ssh.key_path).toBe("/keys/my_key");
    });
    ...
    it("should pass workflows", () => {
      ...
      expect(result.workflows).toEqual(["wf-a"]);
    });

    it("should set SSH port to 22", () => {
      const result = configureSSH("test", {
        host: "remote.com",
        sshUser: "user",
      });
  ...
  describe("configureLocal", () => {
    it("should return a local deployment with correct type", () => {
      const result = configureLocal("test");
    ...
    it("should use custom host", () => {
      const result = configureLocal("test", { host: "0.0.0.0" });
    ...
    it("should use custom port", () => {
      const result = configureLocal("test", { port: 3000 });
      expect(result.port).toBe(3000);
    });

    it("should generate service name from port", () => {
      const result = configureLocal("test", { port: 5000 });
      expect(result.service_name).toBe("nodetool-5000");
    });

    it("should use custom service name", () => {
      const result = configureLocal("test", { serviceName: "custom-svc" });
    ...
    it("should pass GPU setting", () => {
      const result = configureLocal("test", { gpu: "0" });
      expect(result.gpu).toBe("0");
    });
  });
