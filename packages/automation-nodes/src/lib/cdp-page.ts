...
export async function launchBrowser(opts: LaunchOptions = {}): Promise<{
  ...
  if (!port) {
    ...
    chrome = await launch({
      ...
      // Honor CHROME_PATH when set (sandbox container points at
      // /usr/bin/chromium); otherwise fall back to chrome-launcher's own
      // platform-aware discovery for local dev.
