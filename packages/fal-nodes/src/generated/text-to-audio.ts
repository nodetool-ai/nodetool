...
audio, generation, text-to-audio, sound`;
  ...
  static readonly outputTypes = { "audio_file": "str" };
  static readonly falUnitPricing: FalUnitPricing | null = {
    endpointId: "cassetteai/sound-effects-generator",
    unitPrice: 0.00125,
    billingUnit: "compute seconds",
    currency: "USD",
  };
...
export class GeminiTts extends FalNode {
  static readonly nodeType = "fal.text_to_audio.GeminiTts";
  static readonly title = "Gemini Tts";
  static readonly description = `Use Gemini TTS Models to convert your prompts to real audio.
text-to-speech, audio, gemini`;
  static readonly requiredSettings = ["FAL_API_KEY"];
  static readonly outputTypes = { "audio": "audio" };
  static readonly falUnitPricing: FalUnitPricing | null = {
    endpointId: "fal-ai/gemini-tts",
    unitPrice: 1,
  ...
  @prop({ type: "enum", default: "mp3", values: ["wav", "mp3", "ogg_opus"], description: "Audio output format. mp3: compressed, small file size (recommended). wav: uncompressed PCM wrapped in WAV (24 kHz, 16-bit mono). ogg_opus: Ogg container with Opus codec, good quality-to-size ratio." })
  declare output_format: any;

  @prop({ type: "enum", default: "gemini-2.5-flash-tts", values: ["gemini-2.5-flash-tts", "gemini-2.5-pro-tts"], description: "Which Gemini TTS model to use. gemini-2.5-flash-tts: low latency, cost-efficient for everyday applications (recommended). gemini-2.5-pro-tts: highest quality, best for structured workflows like podcasts, audiobooks, and customer support." })
  declare model: any;

  @prop({ type: "enum", default: "Kore", values: ["Achernar", "Achird", "Algenib", "Algieba", "Alnilam", "Aoede", "Autonoe", "Callirrhoe", "Charon", "Despina", "Enceladus", "Erinome", "Fenrir", "Gacrux", "Iapetus", "Kore", "Laomedeia", "Leda", "Orus", "Pulcherrima", "Puck", "Rasalgethi", "Sadachbia", "Sadaltager", "Schedar", "Sulafat", "Umbriel", "Vindemiatrix", "Zephyr", "Zubenelgenubi"], description: "Voice preset for single-speaker synthesis. 30 distinct voices are available. Ignored when speakers is set. Popular choices: Kore (strong, firm female), Puck (upbeat, lively male), Charon (calm, professional male), Zephyr (bright, clear female), Aoede (warm, melodic female)." })
  declare voice: any;

  @prop({ type: "str", default: "", description: "Language for multilingual synthesis. When set, steers the model to speak in the specified language. Supports 24 GA languages and 60+ Preview languages. If not set, the model auto-detects the language from the text." })
  declare language_code: any;

  @prop({ type: "float", default: 1, description: "Controls the randomness of the speech output. Higher values produce more creative and varied delivery, while lower values make the output more predictable and focused." })
  ...
  declare style_instructions: any;
  ...
  async process(): Promise<Record<string, unknown>> {
    ...
    const model = String(this.model ?? "gemini-2.5-flash-tts");
    const voice = String(this.voice ?? "Kore");
    const languageCode = String(this.language_code ?? "");
    const temperature = Number(this.temperature ?? 1);
    const styleInstructions = String(this.style_instructions ?? "");
    ...
    const res = await falSubmit(apiKey, "fal-ai/gemini-tts", args);
    return { output: { type: "audio", uri: (res.audio as any).url } };
  }
}
...
audio, generation, text-to-audio, tts, professional`;
  ...
  static readonly outputTypes = { "audio": "audio" };
  static readonly falUnitPricing: FalUnitPricing | null = {
    endpointId: "fal-ai/minimax-music/v1.5",
    unitPrice: 0.03,
    billingUnit: "generations",
    currency: "USD",
  };
...
audio, generation, text-to-audio, tts, professional`;
  ...
  static readonly outputTypes = { "audio": "audio" };
  static readonly falUnitPricing: FalUnitPricing | null = {
    endpointId: "fal-ai/minimax-music/v2",
    unitPrice: 0.03,
    billingUnit: "generations",
    currency: "USD",
  };
...
audio, generation, text-to-audio, tts`;
  ...
  static readonly outputTypes = { "seed": "int", "audio": "audio" };
  static readonly falUnitPricing: FalUnitPricing | null = {
    endpointId: "fal-ai/stable-audio-25/text-to-audio",
    unitPrice: 0.2,
    billingUnit: "audios",
    currency: "USD",
  };
...
audio, generation, text-to-audio, sound`;
  static readonly requiredSettings = ["FAL_API_KEY"];
  static readonly outputTypes = { "tags": "str", "seed": "int", "lyrics": "str", "audio": "audio" };
  static readonly falUnitPricing: FalUnitPricing | null = {
