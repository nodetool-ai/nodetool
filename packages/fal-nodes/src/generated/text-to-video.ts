...
video, generation, animatediff, animation, text-to-video, txt2vid`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const videoSize = String(inputs.video_size ?? this.video_size ?? "square");
    ...
    const numFrames = Number(inputs.num_frames ?? this.num_frames ?? 16);
...
video, generation, animatediff, sparsectrl, lcm, animation, text-to-video`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const controlnetType = String(inputs.controlnet_type ?? this.controlnet_type ?? "scribble");
    const keyframe_2Index = Number(inputs.keyframe_2_index ?? this.keyframe_2_index ?? 0);
    const keyframe_0Index = Number(inputs.keyframe_0_index ?? this.keyframe_0_index ?? 0);
    const keyframe_1Index = Number(inputs.keyframe_1_index ?? this.keyframe_1_index ?? 0);
    ...
    const keyframe_1ImageRef = inputs.keyframe_1_image as Record<string, unknown> | undefined;
    if (isRefSet(keyframe_1ImageRef)) {
      const keyframe_1ImageUrl = await assetToFalUrl(apiKey, keyframe_1ImageRef!);
      if (keyframe_1ImageUrl) args["keyframe_1_image_url"] = keyframe_1ImageUrl;
    }

    const keyframe_2ImageRef = inputs.keyframe_2_image as Record<string, unknown> | undefined;
    if (isRefSet(keyframe_2ImageRef)) {
      const keyframe_2ImageUrl = await assetToFalUrl(apiKey, keyframe_2ImageRef!);
      if (keyframe_2ImageUrl) args["keyframe_2_image_url"] = keyframe_2ImageUrl;
    }

    const keyframe_0ImageRef = inputs.keyframe_0_image as Record<string, unknown> | undefined;
    if (isRefSet(keyframe_0ImageRef)) {
      const keyframe_0ImageUrl = await assetToFalUrl(apiKey, keyframe_0ImageRef!);
...
video, generation, kling, text-to-video, txt2vid`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const prompt = String(inputs.prompt ?? this.prompt ?? "");
    const duration = String(inputs.duration ?? this.duration ?? "5");
    const advancedCameraControl = String(inputs.advanced_camera_control ?? this.advanced_camera_control ?? "");
    const aspectRatio = String(inputs.aspect_ratio ?? this.aspect_ratio ?? "16:9");
    const cameraControl = String(inputs.camera_control ?? this.camera_control ?? "");
...
video, generation, turbo, fast, text-to-video, txt2vid`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const guidanceScale = Number(inputs.guidance_scale ?? this.guidance_scale ?? 7.5);
    const seed = String(inputs.seed ?? this.seed ?? "");
    const exportFps = Number(inputs.export_fps ?? this.export_fps ?? 8);
    const numFrames = Number(inputs.num_frames ?? this.num_frames ?? 16);
    const numInferenceSteps = Number(inputs.num_inference_steps ?? this.num_inference_steps ?? 4);
...
video, generation, text-to-video, txt2vid`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const prompt = String(inputs.prompt ?? this.prompt ?? "");
    const aspectRatio = String(inputs.aspect_ratio ?? this.aspect_ratio ?? "16:9");
    const tauVideo = Number(inputs.tau_video ?? this.tau_video ?? 0.4);
    const useApg = Boolean(inputs.use_apg ?? this.use_apg ?? true);
    const guidanceScale = Number(inputs.guidance_scale ?? this.guidance_scale ?? 7.5);
    const seed = String(inputs.seed ?? this.seed ?? "");
    const numInferenceSteps = Number(inputs.num_inference_steps ?? this.num_inference_steps ?? 50);
    const negativePrompt = String(inputs.negative_prompt ?? this.negative_prompt ?? "");
...
video, generation, text-to-video, txt2vid`;
  ...
  @prop({ type: "enum", default: "480p", values: ["480p"], description: "The resolution of the output video" })
  declare resolution: any;
  ...
  @prop({ type: "int", default: -1, description: "Random seed for reproducible generation. If not provided, a random seed will be used." })
  declare seed: any;
...
video, generation, text-to-video, txt2vid`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const enablePromptExpansion = Boolean(inputs.enable_prompt_expansion ?? this.enable_prompt_expansion ?? false);
    const seed = String(inputs.seed ?? this.seed ?? "");
    const numFrames = Number(inputs.num_frames ?? this.num_frames ?? 78);
...
video, generation, text-to-video, txt2vid`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    const apiKey = getFalApiKey(inputs);
    const shift = Number(inputs.shift ?? this.shift ?? 10.5);
    const prompt = String(inputs.prompt ?? this.prompt ?? "");
    const maskClampUpper = Number(inputs.mask_clamp_upper ?? this.mask_clamp_upper ?? 0.75);
    const fps = Number(inputs.fps ?? this.fps ?? 16);
    const enableSafetyChecker = Boolean(inputs.enable_safety_checker ?? this.enable_safety_checker ?? true);
    const numFrames = Number(inputs.num_frames ?? this.num_frames ?? 81);
    const maskClampLower = Number(inputs.mask_clamp_lower ?? this.mask_clamp_lower ?? 0.1);
    const maskBinarizationThreshold = Number(inputs.mask_binarization_threshold ?? this.mask_binarization_threshold ?? 0.8);
    const binarizeMask = Boolean(inputs.binarize_mask ?? this.binarize_mask ?? false);
    const videoWriteMode = String(inputs.video_write_mode ?? this.video_write_mode ?? "balanced");
    const resolution = String(inputs.resolution ?? this.resolution ?? "480p");
    const videoOutputType = String(inputs.video_output_type ?? this.video_output_type ?? "VP9 (.webm)");
    const aspectRatio = String(inputs.aspect_ratio ?? this.aspect_ratio ?? "16:9");
    const sampler = String(inputs.sampler ?? this.sampler ?? "euler");
    const videoQuality = String(inputs.video_quality ?? this.video_quality ?? "high");
    const syncMode = Boolean(inputs.sync_mode ?? this.sync_mode ?? false);
    const enablePromptExpansion = Boolean(inputs.enable_prompt_expansion ?? this.enable_prompt_expansion ?? false);
    const seed = String(inputs.seed ?? this.seed ?? "");
...
video, generation, text-to-video, txt2vid`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const prompt = String(inputs.prompt ?? this.prompt ?? "");
    const aspectRatio = String(inputs.aspect_ratio ?? this.aspect_ratio ?? "3:2");
    const resolution = String(inputs.resolution ?? this.resolution ?? "768x512");
    const duration = String(inputs.duration ?? this.duration ?? "5s");
...
video, generation, text-to-video, txt2vid, fast`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const autoFix = Boolean(inputs.auto_fix ?? this.auto_fix ?? true);
    const generateAudio = Boolean(inputs.generate_audio ?? this.generate_audio ?? true);
    const aspectRatio = String(inputs.aspect_ratio ?? this.aspect_ratio ?? "16:9");
    const resolution = String(inputs.resolution ?? this.resolution ?? "720p");
...
video, generation, text-to-video, txt2vid`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    const apiKey = getFalApiKey(inputs);
    const prompt = String(inputs.prompt ?? this.prompt ?? "");
    const resolution = String(inputs.resolution ?? this.resolution ?? "992x512");
    const seed = String(inputs.seed ?? this.seed ?? "");
    const numInferenceSteps = Number(inputs.num_inference_steps ?? this.num_inference_steps ?? 30);
...
video, generation, text-to-video, txt2vid`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const prompt = String(inputs.prompt ?? this.prompt ?? "");
    const aspectRatio = String(inputs.aspect_ratio ?? this.aspect_ratio ?? "16:9");
    const style = String(inputs.style ?? this.style ?? "general");
    const seed = String(inputs.seed ?? this.seed ?? "");
    const movementAmplitude = String(inputs.movement_amplitude ?? this.movement_amplitude ?? "auto");
...
video, generation, text-to-video, txt2vid`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const voiceLanguage = String(inputs.voice_language ?? this.voice_language ?? "en");
...
video, generation, text-to-video, txt2vid`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    const apiKey = getFalApiKey(inputs);
    const duration = String(inputs.duration ?? this.duration ?? "5");
    const effectScene = String(inputs.effect_scene ?? this.effect_scene ?? "");
...
video, generation, text-to-video, txt2vid, fast`;
  ...
  async process(inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const prompt = String(inputs.prompt ?? this.prompt ?? "");
    const condAug = Number(inputs.cond_aug ?? this.cond_aug ?? 0.02);
    const deepCache = String(inputs.deep_cache ?? this.deep_cache ?? "none");
    const fps = Number(inputs.fps ?? this.fps ?? 10);
    const motionBucketId = Number(inputs.motion_bucket_id ?? this.motion_bucket_id ?? 127);
    const videoSize = String(inputs.video_size ?? this.video_size ?? "landscape_16_9");
    const steps = Number(inputs.steps ?? this.steps ?? 20);
    const seed = Number(inputs.seed ?? this.seed ?? -1);
    const negativePrompt = String(inputs.negative_prompt ?? this.negative_prompt ?? "unrealistic, saturated, high contrast, big nose, painting, drawing, sketch, cartoon, anime, manga, render, CG, 3d, watermark, signature, label");
