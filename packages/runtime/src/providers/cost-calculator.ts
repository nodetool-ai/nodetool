...
export interface UsageInfo {
  ...
  inputTokens?: number;
  outputTokens?: number;
  /** Text tokens supplied to an image model. */
  inputTextTokens?: number;
  /** Image tokens supplied to an image model. */
  inputImageTokens?: number;
  /** Image tokens generated by an image model. */
...
function gptImageQualityTier(
  ...
  if (!/^gpt-image-1(?:$|-)/.test(lower)) return null;
...
/**
 * GPT Image 2.5 token pricing in USD.
 * Source: https://developers.openai.com/api/docs/pricing
 */
function gptImage25TokenCost(
  modelId: string,
  usage: UsageInfo
): number | null {
  if (
  ...
  const unattributedInputTokens = Math.max(
    0,
    (usage.inputTokens ?? 0) - inputTextTokens - inputImageTokens
  );
  const outputImageTokens = usage.outputImageTokens ?? usage.outputTokens ?? 0;

  return (
    (inputTextTokens * 5 +
      (inputImageTokens + unattributedInputTokens) * 8 +
...
export class CostCalculator {
  ...
  static calculate(
    ...
    const gptImage25Cost = gptImage25TokenCost(modelId, usage);
    if (gptImage25Cost !== null) return gptImage25Cost;
