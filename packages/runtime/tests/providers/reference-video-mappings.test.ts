...
function atlas() {
    ...
    async (url: string | URL | Request, init?: RequestInit) => {
      if (String(url).endsWith("/uploadMedia")) {
        ...
        return Response.json({
          url: `https://uploads.atlas/reference-${uploadCalls}`
        });
      }
      ...
      if (String(url).includes("/prediction/job")) {
        return Response.json({
          data: {
            status: "completed",
            outputs: ["https://cdn.atlascloud.ai/result.mp4"]
          }
        });
      }
  ...
  return {
    bodies,
    fetchMock,
    provider: new AtlasCloudProvider({ ATLASCLOUD_API_KEY: "test" })
  };
}
...
describe("AtlasCloud reference video requests", () => {
  it.each([
    ...
    "preserves separate reference arrays and video MIME: %j",
    async (inputs) => {
      ...
      if (inputs.images.length)
        expect(bodies[0]).toHaveProperty(
          "reference_images",
          inputs.images.map((bytes) => uri(bytes, "image/png"))
        );
      ...
      if (inputs.videos.length)
        expect(bodies[0]).toHaveProperty(
          "reference_videos",
          inputs.videos.map(
            (_, index) => `https://uploads.atlas/reference-${index + 1}`
          )
        );
  ...
  it("groups typed references in the declared mixed array", async () => {
    const { provider, bodies } = atlas();
    await provider.referenceToVideo(
      { images: [png, png], videos: [webm] },
      {
        model: model("atlascloud", "alibaba/wan-3.0/reference-to-video"),
    ...
    expect(bodies[0]).toHaveProperty("refers", [
      ...
      { url: uri(png, "image/png"), type: "image" },
      { url: "https://uploads.atlas/reference-1", type: "video" }
    ]);
  ...
  it("maps generic images on a reference endpoint without caller task metadata", async () => {
    ...
    await provider.referenceToVideo(
      { images: [png], videos: [] },
      {
        model: model("atlascloud", "google/veo3.1/reference-to-video"),
        prompt: "Keep the cast"
      }
    );
  ...
  it("rejects unsupported kind and forced start-frame calls before network", async () => {
    ...
    await expect(
      provider.referenceToVideo(
        { images: [png], videos: [webm] },
        { model: selected, prompt: "x" }
      )
    ).rejects.toThrow("reference video");
    await expect(
      provider.imageToVideo(png, { model: selected, prompt: "x" })
...
describe("Replicate reference video requests", () => {
  it.each([
    ...
    "maps the full typed request without source fields: %j",
    async (inputs) => {
      const run = vi
        .spyOn(Replicate.prototype, "run")
        .mockResolvedValue(["data:video/mp4;base64,CQ=="]);
      const provider = new ReplicateProvider({ REPLICATE_API_TOKEN: "test" });
      const controller = new AbortController();
      await expect(
        provider.referenceToVideo(inputs, {
          model: model("replicate", "wan-video/wan-2.7-r2v"),
          prompt: "Keep the cast",
      ...
      expect(run).toHaveBeenCalledWith("wan-video/wan-2.7-r2v", {
        input: {
          ...
          ...(inputs.images.length
            ? {
                reference_images: inputs.images.map((bytes) =>
                  uri(bytes, "image/png")
                )
              }
            : {}),
          ...(inputs.videos.length
            ? {
                reference_videos: inputs.videos.map((bytes) =>
                  uri(bytes, "video/webm")
                )
              }
            : {})
        },
  ...
  it("rejects video references for an image-only model before SDK submission", async () => {
    ...
    await expect(
      provider.referenceToVideo(
        { images: [png], videos: [webm] },
        {
          model: model("replicate", "xai/grok-imagine-r2v"),
          prompt: "x"
        }
      )
  ...
  it("cancels rate-limit waits before another billable submission", async () => {
    ...
    await expect(
      withReplicateRetry("test", run, controller.signal)
    ).rejects.toThrow();
