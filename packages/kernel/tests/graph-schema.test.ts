...
describe("T-MSG-5: Graph input/output schema", () => {
  describe("getInputSchema", () => {
    ...
    it("returns schema for multiple input nodes", () => {
      ...
      expect((schema.properties as Record<string, any>).count.type).toBe(
        "number"
      );
      expect((schema.properties as Record<string, any>).label.type).toBe(
        "string"
      );
    });
    ...
    it("uses node id as name fallback", () => {
      ...
      expect((schema.properties as Record<string, any>).my_input.type).toBe(
        "number"
      );
    });
    ...
    it("maps bool output to boolean json schema type", () => {
      ...
      const schema = graph.getInputSchema();
      expect((schema.properties as Record<string, any>).flag.type).toBe(
        "boolean"
      );
    });
  });
  ...
  describe("getOutputSchema", () => {
    it("returns schema for output nodes", () => {
      ...
      expect((schema.properties as Record<string, any>).result.type).toBe(
        "number"
      );
