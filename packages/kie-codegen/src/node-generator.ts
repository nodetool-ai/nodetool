...
export function generateModule(config: ModuleConfig): string {
  ...
  for (const node of config.nodes) {
    lines.push(renderClass(node, config));
    lines.push(``);
...
function renderClass(node: NodeConfig, moduleConfig: ModuleConfig): string {
  const fullClassName = `${node.className}Node`;
  const nodeType = `kie.${moduleConfig.moduleName}.${node.className}`;
  ...
  for (const field of node.fields) {
    lines.push(renderProp(field));
    lines.push(`  declare ${field.name}: any;`);
    lines.push(``);
  }

  lines.push(...renderProcess(node, pollInterval, maxAttempts));

  lines.push(`}`);
  return lines.join("\n");
}

function renderProp(field: FieldDef): string {
  const parts: string[] = [];
