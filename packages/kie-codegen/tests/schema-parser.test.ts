...
import { parseKieSchema } from "../src/schema-parser.js";
...
const oneOfDocs = `# Vocal & Instrument Stem Separation
              ...
              oneOf:
                - title: Separate using existing audio
                  ...
                  properties:
                    taskId:
                      type: string
                      description: Music generation task id.
                    audioId:
                      type: string
...
const playgroundDocs = `# HappyHorse 1.1 图生视频
...
describe("parseKieSchema", () => {
  it("maps KIE OpenAPI media URL fields to AssetRef handles", () => {
    const node = parseKieSchema(seedanceDocs, {
      category: "Video Models > Bytedance",
      title: "bytedance-seedance-2",
      url: "https://docs.kie.ai/market/bytedance/seedance-2.md",
      summary: "Seedance 2"
  ...
  it("maps direct Suno endpoints to audio useSuno configs", () => {
    const node = parseKieSchema(sunoDocs, {
  ...
  it("maps gemini omni video ID and clip fields", () => {
    const node = parseKieSchema(geminiOmniVideoDocs, {
  ...
  it("maps gemini omni audio direct endpoint to text output", () => {
    const node = parseKieSchema(geminiOmniAudioDocs, {
      category: "Video Models > Gemini",
  ...
  it("maps gemini omni character direct endpoint to text output", () => {
    const node = parseKieSchema(geminiOmniCharacterDocs, {
      category: "Video Models > Gemini",
  ...
  it("collects fields from oneOf request-body branches", () => {
    const node = parseKieSchema(oneOfDocs, {
      category: "Suno API > Vocal Removal",
      title: "Vocal & Instrument Stem Separation",
      url: "https://docs.kie.ai/suno-api/separate-vocals.md",
      summary: "Separate vocals"
    });

    expect(node?.fields.map((field) => field.name)).toEqual([
      "stemName",
      "taskId",
      "audioId",
      "audioUrl"
    ...
    expect(
      node?.fields.find((field) => field.name === "stemName")?.required
    ).toBe(true);
    // The first branch is the node's primary input mode; the second branch's
    // alternative stays optional.
    expect(node?.fields.find((field) => field.name === "taskId")?.required).toBe(
      true
    );
    expect(
      node?.fields.find((field) => field.name === "audioUrl")?.required
    ).toBe(false);
  });

  it("names a node after its model when the operationId is autogenerated", () => {
    const node = parseKieSchema(playgroundDocs, {
      category: "Video Models > HappyHorse",
      title: "HappyHorse 1.1 图生视频",
      url: "https://docs.kie.ai/market/happyhorse/1-1-image-to-video.md",
      summary: "HappyHorse-1-1 image-to-video"
    });
