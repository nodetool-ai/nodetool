...
describe("MCP server surface", () => {
  ...
  it.each(["stdio-local", "local-dev-http"] as const)(
    "reads a directly uploaded asset from the same %s execute_code session",
    async (source) => {
      ...
      const observation = await act(
        server,
        `const listed = await nodetool.assets.list({ limit: 10 });\n` +
  ...
  it.each(["stdio-local", "http-session"] as const)(
    "views a directly uploaded image by short id in a %s session",
    async (source) => {
      ...
      expect(upload.isError).toBeFalsy();
      const saved = JSON.parse(upload.content[0].text) as { asset_id: string };
      const viewed = await callTool(server, "view_image", {
        image_id: saved.asset_id.slice(0, 12)
      });
