...
import { resolveExportSource } from "../lib/asset-export.js";
import {
  writeStoryboardZip,
  type StoryboardExportInput
} from "../lib/storyboard-export.js";
import { streamZip, type ExportSource } from "../lib/zip-stream.js";
...
/**
 * Bytes or a local file for one shot's media ref.
 *
 * `resolveExportSource` covers the stored forms (`asset://`,
...
const storyboardsRoutes: FastifyPluginAsync<RouteOptions> = async (
  ...
  app.get("/api/storyboards/:id/export-zip", async (req, reply) => {
    ...
    await bridge(req, reply, async (request) => {
      ...
      // The archive streams as it is written; a clip read from a local file
      ...
      const safe =
        (board.name || screenplay?.title || "storyboard").replace(
          /[^A-Za-z0-9._-]+/g,
          "_"
        ) || "storyboard";
