...
let setSelectedAssetsMock: jest.Mock;
...
beforeEach(() => {
  ...
  assetGridStore.setState({
    ...
    setSelectedAssets: setSelectedAssetsMock,
...
describe("useAssetSelection", () => {
  test("single click selects asset and clears audio for non-audio", () => {
    const { result } = renderHook(() =>
      useAssetSelection([asset1, asset2, asset3])
    );
  ...
  test("select all and deselect", () => {
    const { result, rerender } = renderHook(() =>
      useAssetSelection([asset1, asset2])
    );
