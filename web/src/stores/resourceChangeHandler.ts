 ...
 *
 * Keys here are matched as prefixes by TanStack Query: `["assets"]`
 * matches `["assets", { parent_id: x }]`, `["assets", { workflow_id: y }]`,
 * etc. — but does NOT match the singular `["asset", id]`. Singular keys
 * are invalidated explicitly below for resources that use them.
 */
...
export function handleResourceChange(update: ResourceChangeUpdate): void {
  ...
  if (resource.id) {
    ...
    // WorkflowVersion changes scope to a parent workflow id when available;
    // otherwise refetch any cached versions list.
