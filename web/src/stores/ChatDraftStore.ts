 ...
 * the user's behalf — the dashboard quick starters, for one — parks the
 * example prompt here; the composer for that thread picks it up on mount and
 * clears it, so the text lands in the box ready to send instead of being sent
 * for the user. Not persisted: a seed that outlives the session would reappear
 * in a conversation the user has moved on from.
 */
