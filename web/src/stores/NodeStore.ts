...
export interface NodeStore {
  ...
  saveWorkflowDebounce: DebouncedFunc<() => void>;
  syncWorkflow: () => void;
  ...
  temporal(
    (set, get) => ({
      ...
      saveWorkflow: async () => {
        ...
        return updateWorkflow(workflow).then(() => {
          devLog("workflow updated: ", workflow);
          get().syncWorkflow();
        });
      },

      /**
       * Debounced save workflow.
       */
      saveWorkflowDebounce: debounce(async () => {
        get().saveWorkflow();
      }, 2000),
