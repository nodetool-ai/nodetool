...
const HuggingFaceModelDownloader: React.FC = () => {
  ...
  const startDownload = useCallback((repoId: string) => {
  ...
  return (
    <Box>
      ...
      {Object.keys(downloads).length > 0 && <Typography variant="h5">Downloads</Typography>}

      {Object.keys(downloads).map((name) =>
      (
        <Box key={name} mt={2} style={{ overflow: "scroll" }}>
          <Typography variant="subtitle1">{name}</Typography>
          <pre style={{ padding: "5px", whiteSpace: "pre-wrap", fontFamily: "monospace", fontSize: "9px", border: "1px solid grey", background: "black" }}>
