...
const MostUsedTiles = memo(function MostUsedTiles() {
  const theme = useTheme();
  const memoizedStyles = useMemo(() => tileStyles(theme), [theme]);

  const { mostUsedNodes, clearMostUsedNodes } = useMostUsedNodesStore(
    (state) => ({
      mostUsedNodes: state.mostUsedNodes,
      clearMostUsedNodes: state.clearMostUsedNodes
    })
  );
  ...
  const handleClearMostUsed = useCallback(() => {
    clearMostUsedNodes();
    addNotification({
      type: "info",
  ...
  }, [clearMostUsedNodes, addNotification]);
  ...
  const filteredMostUsedNodes = useMemo(
    () => mostUsedNodes.filter((node) => !QUICK_ACTION_NODE_TYPES.has(node.nodeType)),
    [mostUsedNodes]
  );

  if (filteredMostUsedNodes.length === 0) {
    return null;
  }
    ...
    <Box css={memoizedStyles}>
      <div className="tiles-header">
        <Typography variant="h5">
          <TrendingUpIcon fontSize="small" sx={{ opacity: 0.8, color: "success.main" }} />
          Most Used
        </Typography>
      ...
      <div className="tiles-container">
        {filteredMostUsedNodes.map((mostUsedNode) => {
          const { nodeType, count } = mostUsedNode;
          ...
          return (
            ...
            >
              ...
              >
                <span className="usage-badge">{count}</span>
