  ...
  return {
    StateIconButton: ({ icon, tooltip, onClick, disabled, isActive, color, className }: any) => (
      <button
        ...
        data-color={color}
        className={className}
        aria-label={typeof tooltip === "string" ? tooltip : "Agent Mode Toggle"}
      >
        {icon}
        {tooltip}
      </button>
...
describe("AgentModeToggle", () => {
  it("should render successfully", () => {
    ...
    renderWithTheme(<AgentModeToggle agentMode={false} onToggle={onToggle} />);
  ...
  it("should render with active state when agentMode is true", () => {
    const onToggle = jest.fn();
    renderWithTheme(<AgentModeToggle agentMode={true} onToggle={onToggle} />);

    const button = screen.getByRole("button");
    expect(button).toHaveAttribute("data-isactive", "true");
    expect(button).toHaveAttribute("data-color", "primary");
    expect(button).toHaveClass("agent-toggle");
    expect(button).toHaveClass("active");
  });

  it("should render with inactive state when agentMode is false", () => {
    const onToggle = jest.fn();
    renderWithTheme(<AgentModeToggle agentMode={false} onToggle={onToggle} />);
  ...
  it("should call onToggle with false when agentMode is true", async () => {
  ...
  it("should call onToggle with true when agentMode is false", async () => {
    const user = userEvent.setup();
    const onToggle = jest.fn();
    renderWithTheme(<AgentModeToggle agentMode={false} onToggle={onToggle} />);
  ...
  it("should be disabled when disabled prop is true", () => {
    const onToggle = jest.fn();
    renderWithTheme(
      <AgentModeToggle agentMode={false} onToggle={onToggle} disabled={true} />
    );
  ...
  it("should not be disabled when disabled prop is false", () => {
    const onToggle = jest.fn();
    renderWithTheme(
      <AgentModeToggle agentMode={false} onToggle={onToggle} disabled={false} />
    );
  ...
  it("should use useCallback for click handler to prevent unnecessary re-renders", () => {
    ...
    const { rerender } = renderWithTheme(
      <AgentModeToggle agentMode={false} onToggle={onToggle} />
    );
  ...
  it("should update handler when agentMode changes", async () => {
    const user = userEvent.setup();
    const onToggle = jest.fn();
    const { rerender } = renderWithTheme(
      <AgentModeToggle agentMode={false} onToggle={onToggle} />
    ...
    // Click with agentMode false
    const button = screen.getByRole("button");
    await user.click(button);
    expect(onToggle).toHaveBeenLastCalledWith(true);

    // Re-render with agentMode true
    rerender(
      <ThemeProvider theme={mockTheme}>
        <AgentModeToggle agentMode={true} onToggle={onToggle} />
      </ThemeProvider>
    );

    // Click with agentMode true
  ...
  it("should render correct tooltip content for active state", () => {
    ...
    const button = screen.getByRole("button");
    expect(button.textContent).toContain("Agent Mode ON");
    expect(button.textContent).toContain("Disable");
  });
  ...
  it("should render correct tooltip content for inactive state", () => {
    ...
    const button = screen.getByRole("button");
    expect(button.textContent).toContain("Agent Mode OFF");
  ...
  it("should update handler when onToggle callback changes", async () => {
    ...
    const { rerender } = renderWithTheme(
      <AgentModeToggle agentMode={false} onToggle={onToggle1} />
    );
    ...
    rerender(
      <ThemeProvider theme={mockTheme}>
        <AgentModeToggle agentMode={false} onToggle={onToggle2} />
      </ThemeProvider>
    );
