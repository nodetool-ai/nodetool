...
- [x] [impl] extract connected shell subscriber components (`ConnectedToolbar`, `ConnectedToolTopBar`, `ConnectedLayersPanel`, `ConnectedContextMenu`, `SketchCanvasPane`) into a dedicated editor-shell module so `SketchEditor.tsx` stops being both the editor root and the place where all shell/store subscriber wrappers live
  - **Done:** Moved all 5 connected shell components into `editor-shell/` directory with individual files: `ConnectedToolbar.tsx`, `ConnectedToolTopBar.tsx`, `ConnectedLayersPanel.tsx`, `ConnectedContextMenu.tsx`, `SketchCanvasPane.tsx`. Created `editor-shell/index.ts` barrel export. SketchEditor.tsx now imports from `./editor-shell` and is focused on session orchestration + layout composition. Added module export test.
...
- [x] [impl+test] extract a transform-UI adapter hook so `ConnectedToolTopBar` and `ConnectedContextMenu` consume one shared transform display/action model instead of separately wiring transform scalars, preview-driven display state, and commit/cancel/reset callbacks through `SketchEditor.tsx`
  - **Done:** This is the same `useTransformAdapter()` hook from the unify task above. Both `ConnectedToolTopBar` and `ConnectedContextMenu` now call `useTransformAdapter({ onTransformCommit, onTransformCancel, onTransformReset })` and consume `transform.display.scaleX`, `transform.display.scaleY`, `transform.display.rotation`, `transform.actions.onCommit`, `transform.actions.onCancel`, `transform.actions.onReset` instead of independently wiring transform scalars and callbacks. Full regression coverage included in the transform adapter tests.
