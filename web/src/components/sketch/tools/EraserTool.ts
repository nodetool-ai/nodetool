/**
 * EraserTool – uses destination-out composite via stroke buffer.
 *
 * Extracted from usePointerHandlers painting-tool sections:
 *   handlePointerDown (~line 980-1092) – eraser-specific path
 *   handlePointerMove (~line 1297-1317)
...
export class EraserTool implements ToolHandler {
  ...
  private eraserStrokeStampStates: Map<number, StrokeStampState> = new Map();

  // Stamp cache (persistent across strokes for perf)
  private eraserStampCache: Map<string, HTMLCanvasElement> = new Map();

  // ── Drawing wrapper ──────────────────────────────────────────────────

  private drawEraserStroke(
    from: Point,
    to: Point,
    ...
    let stampState = this.eraserStrokeStampStates.get(branchIndex);
    if (!stampState) {
      stampState = { hasStamped: false, distanceToNextDab: 0 };
      this.eraserStrokeStampStates.set(branchIndex, stampState);
    }
  ...
  onDown(ctx: ToolContext, event: ToolPointerEvent): boolean | void {
    ...
    this.eraserStrokeStampStates.clear();
    ...
    if (paintCtx) {
      ...
      if (ctx.shiftHeldRef.current && this.lastStrokeEnd) {
        ...
        for (let i = 1; i <= steps; i++) {
          ...
          ctx.withMirror(
            paintCtx,
            (f, to, c, branchIndex) =>
              this.drawEraserStroke(f, to, doc.toolSettings.eraser, c, this.currentPressure, branchIndex),
            prev,
            current
          );
          prev = current;
      ...
      // No initial dab for eraser — the move handler draws it
  ...
  onMove(
    ...
    for (const eventPoint of coalescedPoints) {
      ...
      ctx.withMirror(
        paintCtx,
        (f, t, c, branchIndex) =>
          this.drawEraserStroke(f, t, doc.toolSettings.eraser, c, pressure, branchIndex),
        from,
        smoothPt
      );
      this.lastSmoothedPoint = {
