...
const AssetTree: React.FC<AssetTreeProps> = ({
  ...
  const renderAssetTree = useCallback((nodes: AssetTreeNode[], depth = 0) => {
    // Use the already-sorted nodes from parent, don't re-sort
    return (
      <List
          ...
          <React.Fragment key={node.id}>
            ...
            {node.content_type === "folder" && node.children && (
              <Collapse
                ...
                unmountOnExit
              >
                {renderAssetTree(sortNodes(node.children), depth + 1)}
              </Collapse>
            )}
          </React.Fragment>
        ))}
  ...
  }, [closedFolders, createFolderToggleHandler, getFileIcon, sortNodes, theme]);
