...
export type AssetRefOrDivider =
  | { isDivider: true; type: string; count: number }
  | (AssetRef & { isDivider: false; type: string });
...
export const prepareItems = (
  ...
  const assetsByType: Record<string, AssetRef[]> = {
    image: [],
    audio: [],
    video: [],
    text: [],
  ...
  assetRefs.forEach((assetRef) => {
    const type = assetRef.type;
    if (type in assetsByType) {
      assetsByType[type].push(assetRef);
    } else {
      assetsByType.other.push(assetRef);
    }
  });

  return Object.entries(assetsByType).flatMap(
    ([type, assets]): AssetRefOrDivider[] => {
      if (assets.length === 0) {
        return [];
      }
      return [
...
export const calculateRowCount = (
  preparedItems: AssetRefOrDivider[],
  columns: number
): number => {
  let count = 0;
