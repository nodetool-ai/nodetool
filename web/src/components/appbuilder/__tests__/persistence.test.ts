...
import { AppDocument, APP_SCHEMA_VERSION } from "../appData";
...
describe("appbuilder persistence", () => {
  it("round-trips an application document through the app_doc field", () => {
    ...
    expect((data!.content[0] as { props: { label: string } }).props.label).toBe(
      "Go"
    );
