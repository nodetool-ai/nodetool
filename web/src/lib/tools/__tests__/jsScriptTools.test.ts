...
describe("ui_jsscript_* tools", () => {
  ...
  it("returns the snapshot through the handler", async () => {
    ...
    const result = await callTool<{ ok: boolean } & JsScriptSnapshot>(
      "ui_jsscript_get_state",
      { script_id: SCRIPT_ID },
      "tc-2",
      ctx
  ...
  it("replaces the body and reports the validation issues", async () => {
    ...
    const result = await callTool<{ ok: boolean; chars: number; issues: { code: string }[] }>(
      "ui_jsscript_set_code",
  ...
  it("runs the script with the given inputs", async () => {
    ...
    const result = await callTool<{ ok: boolean; run: { outputs?: Record<string, unknown> } }>(
      "ui_jsscript_run",
  ...
  it("mirrors a failed run on the tool result", async () => {
    ...
    const result = await callTool<{ ok: boolean; run: { ok: boolean; error?: string } }>(
      "ui_jsscript_run",
      { script_id: SCRIPT_ID },
      "tc-6c",
      ctx
  ...
  it("returns the graded report from the test tool", async () => {
    ...
    const result = await callTool<{ ok: boolean; passed: number; failed: number }>(
      "ui_jsscript_test",
      { script_id: SCRIPT_ID },
      "tc-8",
      ctx
