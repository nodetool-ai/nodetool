...
describe("ui_timeline_* tools", () => {
  ...
  it("exposes split_clip's parameter schema with target required", () => {
    ...
    const schema = toolParameterSchema("ui_timeline_split_clip");
  ...
  it("returns the timeline snapshot through the handler", async () => {
    ...
    const result = await callTool<{ ok: boolean } & TimelineSnapshot>(
      "ui_timeline_get_state",
      { timeline_id: SEQ_ID },
      "tc-2",
      ctx
  ...
  it("generates a clip via the handler", async () => {
    ...
    const result = await callTool<{ ok: boolean; clip: TimelineClipNode; generationStarted: boolean }>(
      "ui_timeline_generate_clip",
      {
        timeline_id: SEQ_ID,
        kind: "text-to-video",
  ...
  it("places an existing asset as a clip", async () => {
    ...
    const result = await callTool<{ ok: boolean; clip: { name: string } }>(
  ...
  it("splits a clip at a time through the handler", async () => {
    ...
    const result = await callTool<{ ok: boolean; clips: TimelineClipNode[] }>(
      "ui_timeline_split_clip",
  ...
  it("gets video frames through the handler", async () => {
    ...
    const result = await callTool<{ ok: boolean; frames: TimelineClipFrameNode[] }>(
      "ui_timeline_get_clip_frames",
  ...
  it("seeks the playhead through the handler", async () => {
    ...
    const result = await callTool<{ ok: boolean; playheadMs: number }>(
      "ui_timeline_seek",
