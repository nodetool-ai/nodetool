/**
 * `ui_app_*` frontend tools: let the agent read and edit the open App Builder
...
FrontendToolRegistry.register({
  ...
  description:
    "Read the open App Builder: every placed widget (id, type, props, parent), " +
    "the selected widget, the page title, and the available widget types. Call " +
    "this first to see what's on the page before editing.",
  parameters: z.object({}),
  async execute() {
    return { ok: true, ...getPuckAgentHandler().getSnapshot() };
  }
});
...
FrontendToolRegistry.register({
  ...
  parameters: z.object({}),
  async execute() {
    return { ok: true, types: getPuckAgentHandler().listComponentTypes() };
  }
});
...
FrontendToolRegistry.register({
  ...
  parameters: z.object({
    ...
    parent_id: z
      ...
      .nullable()
      .optional()
      .describe("Id of a Panel/Columns widget to nest inside."),
    slot: z
      .string()
      .nullable()
      .optional()
  ...
  async execute({ type, props, parent_id, slot, index }) {
    const component = getPuckAgentHandler().addComponent({
      type,
...
FrontendToolRegistry.register({
  ...
  parameters: z.object({
    id: z.string().describe("Widget id from ui_app_get_snapshot."),
    props: z.record(z.string(), z.unknown()).describe("Props to merge.")
  }),
  async execute({ id, props }) {
    const component = getPuckAgentHandler().updateComponent(
      id,
...
FrontendToolRegistry.register({
  ...
  async execute({ id }) {
    const removed = getPuckAgentHandler().removeComponent(id);
...
FrontendToolRegistry.register({
  ...
  async execute({ id }) {
    getPuckAgentHandler().selectComponent(id ?? null);
    return { ok: true };
  }
});
...
FrontendToolRegistry.register({
  ...
  async execute({ title }) {
    getPuckAgentHandler().setRootProps({ title });
