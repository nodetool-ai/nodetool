...
FrontendToolRegistry.register({
  ...
  async execute({ kind, limit }) {
    ...
    const raw = await trpc.models.availableForKind.query({ kind });
    // The output schema promises an array; `?? []` covers a server that
    // answers `null` anyway, which searchModelsTool.test.ts pins.
