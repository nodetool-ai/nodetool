...
import type { RunPodJobResponse } from "../services/RunPodService";
...
export async function cancelComfyUIExecution(promptId: string): Promise<void> {
  ...
  if (store.backendType === "runpod") {
    const runpodService = getRunPodService();
    if (runpodService) {
      await runpodService.cancelJob(promptId);
    }
  } else {
    const service = getComfyUIService();
...
let runpodServiceInstance: RunPodService | null = null;

function getRunPodService(): RunPodService | null {
  const { runpod } = useComfyUIStore.getState();
  if (!runpod.apiKey || !runpod.endpointId) {
    return null;
  }
  if (
    runpodServiceInstance &&
    runpodServiceInstance.getEndpointId() === runpod.endpointId
  ) {
    runpodServiceInstance.setApiKey(runpod.apiKey);
    return runpodServiceInstance;
  }
  runpodServiceInstance = new RunPodService(runpod.apiKey, runpod.endpointId);
  return runpodServiceInstance;
}

/**
 * Execute a ComfyUI workflow via RunPod serverless endpoint.
 * Submits the prompt, then polls for status, translating updates
 * into the same workflow-update format used by the local ComfyUI bridge.
 */
export async function executeViaRunPod(
  graph: Graph,
  onProgress?: (_progress: { type: string; data: unknown }) => void,
  workflowForUpdates?: Workflow
): Promise<{
  success: boolean;
  promptId?: string;
  ...
  const service = getRunPodService();
  if (!service) {
    return { success: false, error: "RunPod is not configured (missing API key or endpoint ID)" };
  }

  try {
    const prompt = nodeToolGraphToComfyPrompt(graph);
    log.info("[RunPod] Submitting workflow to RunPod...");

    const submitResponse = await service.submitWorkflow(prompt);
    const jobId = submitResponse.id;

    log.info("[RunPod] Job submitted:", jobId);
    useComfyUIStore.getState().setCurrentPromptId(jobId);
    useComfyUIStore.getState().setExecuting(true);
    ...
    const finalResponse = await service.pollUntilComplete(
      jobId,
      (statusResponse: RunPodJobResponse) => {
        if (statusResponse.status !== lastStatus) {
          lastStatus = statusResponse.status;
          log.info("[RunPod] Status:", statusResponse.status);

          if (onProgress) {
            onProgress({
              type: `runpod_${statusResponse.status.toLowerCase()}`,
              data: statusResponse
            });
          }

          if (workflowForUpdates && statusResponse.status === "IN_PROGRESS") {
            emitWorkflowUpdate(workflowForUpdates, {
              type: "job_update",
              status: "running",
              job_id: jobId,
              workflow_id: workflowForUpdates.id,
              message: "RunPod job in progress"
            } as MsgpackData);
          }
        }
      }
    ...
    if (finalResponse.status === "COMPLETED") {
      log.info("[RunPod] Job completed successfully");

      if (workflowForUpdates && finalResponse.output) {
        emitRunPodOutputs(workflowForUpdates, graph, finalResponse);
      }
      ...
      return { success: true, promptId: jobId };
    }

    const errorMsg =
      finalResponse.error ||
      finalResponse.output?.errors?.join("; ") ||
      `RunPod job ${finalResponse.status}`;
    ...
    return { success: false, promptId: jobId, error: errorMsg };
  } catch (error) {
    const errorMessage =
      error instanceof Error ? error.message : "Unknown RunPod error";
    log.error("[RunPod] Execution failed:", error);
    useComfyUIStore.getState().setExecuting(false);
...
/**
 * Emit output images from a completed RunPod job as workflow updates.
 * RunPod ComfyUI worker returns images as base64 or S3 URLs.
 */
function emitRunPodOutputs(
  workflow: Workflow,
  graph: Graph,
  response: RunPodJobResponse
): void {
  ...
  // Find the first output node (SaveImage etc.) to attribute results to,
  // or fall back to the first node in the graph.
  const outputNodeId = graph.nodes[0]?.id || "0";

  const nodeLookup = buildNodeLookup(graph);
  const node = getNodeMetadata(nodeLookup, outputNodeId);
  ...
  const imageValues = images.map((img) => {
    ...
    // s3_url — data field contains the URL
    return {
      filename: img.filename,
      type: "image",
      url: img.data
  ...
  emitWorkflowUpdate(workflow, {
    ...
    node_type: node.type,
    status: "completed",
    result: { images: imageValues },
    workflow_id: workflow.id
  } as MsgpackData);
}

/**
 * Execute a ComfyUI workflow, routing to RunPod or local based on store config.
 */
export async function executeComfyWorkflow(
  ...
  const { backendType } = useComfyUIStore.getState();
  if (backendType === "runpod") {
    return executeViaRunPod(graph, onProgress, workflowForUpdates);
  }
  return executeViaComfyUI(graph, onProgress, workflowForUpdates);
}
