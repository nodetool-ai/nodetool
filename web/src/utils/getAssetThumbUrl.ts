 ...
 */
export const getAssetThumbUrl = (asset: Asset, fallbackUrl: string = "/images/placeholder.png"): string => {
  // Case 1: Asset has a thumbnail URL
  if (asset.thumb_url) {
    return asset.thumb_url;
  }
  
  // Case 2: Asset has binary data
  if (asset.data && asset.type === "image") {
    try {
      // Convert to Uint8Array if it's not already
      const uint8Array = asset.data instanceof Uint8Array 
        ? asset.data 
