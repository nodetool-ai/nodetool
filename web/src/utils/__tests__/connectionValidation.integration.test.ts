...
describe("Connection Validation Integration Tests", () => {
  describe("Static Node Connections", () => {
    it("should validate compatible static connections", () => {
      ...
      // Find handles
      const sourceHandle = findOutputHandle(textNode, "processed_text", textProcessorMetadata);
      const targetHandle = findInputHandle(dynamicNode, "static_input", dynamicProcessorMetadata);

      expect(sourceHandle).toBeDefined();
      expect(targetHandle).toBeDefined();

      // Validate type compatibility
      const isCompatible = isConnectable(sourceHandle!.type, targetHandle!.type);
    ...
    it("should reject incompatible static connections", () => {
      ...
      const sourceHandle = findOutputHandle(textNode, "word_count", textProcessorMetadata);
      const targetHandle = findInputHandle(mathNode, "operand_a", mathCalculatorMetadata);
    ...
    it("should reject connections to non-existent handles", () => {
      ...
      const invalidOutput = findOutputHandle(textNode, "non_existent_output", textProcessorMetadata);
      const invalidInput = findInputHandle(textNode, "non_existent_input", textProcessorMetadata);
  ...
  describe("Dynamic Node Connections", () => {
    it("should validate connections to dynamic properties", () => {
      ...
      // Find handles
      const sourceHandle = findOutputHandle(textNode, "processed_text", textProcessorMetadata);
      const targetHandle = findInputHandle(dynamicNode, "custom_input", dynamicProcessorMetadata);

      expect(sourceHandle).toBeDefined();
      expect(targetHandle).toBeDefined();
      expect(targetHandle!.isDynamic).toBe(true);

      // Dynamic properties default to string type
    ...
    it("should validate connections from dynamic outputs", () => {
      ...
      // Find handles
      const sourceHandle = findOutputHandle(dynamicNode, "custom_output", dynamicProcessorMetadata);
      const targetHandle = findInputHandle(textNode, "case_sensitive", textProcessorMetadata);
    ...
    it("should reject connections to non-existent dynamic properties", () => {
      ...
      const sourceHandle = findOutputHandle(textNode, "processed_text", textProcessorMetadata);
      const targetHandle = findInputHandle(dynamicNode, "non_existent_prop", dynamicProcessorMetadata);
    ...
    it("should reject dynamic properties on non-dynamic nodes", () => {
      ...
      const targetHandle = findInputHandle(textNode, "dynamic_prop", textProcessorMetadata);
      expect(targetHandle).toBeUndefined();
    });
  });
  ...
  describe("Complete Handle Discovery", () => {
    it("should discover all handles on static nodes", () => {
      ...
      expect(allOutputs.map(h => h.name)).toEqual(["processed_text", "word_count"]);
      expect(allOutputs.every(h => !h.isDynamic)).toBe(true);

      expect(allInputs).toHaveLength(2);
      expect(allInputs.map(h => h.name)).toEqual(["input_text", "case_sensitive"]);
    ...
    it("should discover all handles on dynamic nodes with dynamic properties/outputs", () => {
      ...
      const allOutputs = getAllOutputHandles(dynamicNode, dynamicProcessorMetadata);
      const allInputs = getAllInputHandles(dynamicNode, dynamicProcessorMetadata);
      ...
      expect(allOutputs.filter(h => h.isDynamic).map(h => h.name)).toEqual(["dyn_out1", "dyn_out2"]);
      ...
      expect(allInputs.filter(h => h.isDynamic).map(h => h.name)).toEqual(["dyn_prop1", "dyn_prop2"]);
    });
  });
  ...
  describe("Type Compatibility Matrix", () => {
    ...
    testCases.forEach(({ source, target, expected }) => {
      it(`should ${expected ? 'allow' : 'reject'} connection from ${source.type} to ${target.type}`, () => {
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should prioritize static handles over dynamic ones with same names", () => {
      ...
      const inputHandle = findInputHandle(node, "static_input", dynamicProcessorMetadata);
      const outputHandle = findOutputHandle(node, "static_output", dynamicProcessorMetadata);
  ...
  describe("Real-world Connection Scenarios", () => {
    it("should handle a complex workflow with mixed static and dynamic connections", () => {
      ...
      const conn1Source = findOutputHandle(textNode, "processed_text", textProcessorMetadata);
      const conn1Target = findInputHandle(dynamicNode, "text_input", dynamicProcessorMetadata);
      ...
      const conn2Source = findOutputHandle(textNode, "word_count", textProcessorMetadata);
      const conn2Target = findInputHandle(dynamicNode, "numeric_input", dynamicProcessorMetadata);
      
      expect(conn2Source).toBeDefined();
      expect(conn2Target).toBeDefined();
      expect(conn2Target!.isDynamic).toBe(true);
      // This should fail because int != str (dynamic properties default to str)
      expect(isConnectable(conn2Source!.type, conn2Target!.type)).toBe(false);

      // Connection 3: DynamicProcessor.processed_count (dynamic) -> MathCalculator.operand_a
      const conn3Source = findOutputHandle(dynamicNode, "processed_count", dynamicProcessorMetadata);
      const conn3Target = findInputHandle(mathNode, "operand_a", mathCalculatorMetadata);
    ...
    it("should validate a complete connection workflow end-to-end", () => {
      ...
      const targetNode = createTestNode("target", "dynamic.processor", { "text_prop": "value" });

      // 1. Find source handle
      const sourceHandle = findOutputHandle(sourceNode, "processed_text", textProcessorMetadata);
      expect(sourceHandle).toBeDefined();

      // 2. Find target handle  
      const targetHandle = findInputHandle(targetNode, "text_prop", dynamicProcessorMetadata);
      expect(targetHandle).toBeDefined();
      ...
      const connectionIsValid = sourceHandle !== undefined && 
