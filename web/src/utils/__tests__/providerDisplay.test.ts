...
describe("providerDisplay", () => {
  ...
  describe("isHuggingFaceLocalProvider", () => {
    ...
    it("should return false for HuggingFace with sub-org", () => {
      ...
      expect(isHuggingFaceLocalProvider("huggingface-nvidia")).toBe(false);
    });

    it("should return false for non-HuggingFace providers", () => {
      expect(isHuggingFaceLocalProvider("openai")).toBe(false);
      expect(isHuggingFaceLocalProvider("ollama")).toBe(false);
  ...
  describe("isLocalProvider", () => {
    it("should return true for HuggingFace local provider", () => {
  ...
  describe("isCloudProvider", () => {
    ...
    it("should return true for HuggingFace with sub-org", () => {
      expect(isCloudProvider("huggingface/fireworks-ai")).toBe(true);
      expect(isCloudProvider("huggingface_cerebras")).toBe(true);
    });
  ...
  describe("isHuggingFaceInferenceProvider", () => {
    it("should return true for hf_inference variants", () => {
      ...
      expect(isHuggingFaceInferenceProvider("hf_inference_endpoint")).toBe(true);
    });
    ...
    it("should return true for huggingface_inference variants", () => {
      ...
      expect(isHuggingFaceInferenceProvider("huggingface_inference_api")).toBe(true);
    });
  ...
  describe("getModelUrl", () => {
    it("should return HuggingFace URL for HF models", () => {
    ...
    it("should return Ollama URL for Ollama models", () => {
      expect(getModelUrl("ollama", "llama2")).toBe("https://ollama.com/library/llama2");
    ...
    it("should return Google docs URL for Google/Gemini", () => {
      expect(getModelUrl("gemini", "gemini-pro")).toBe("https://ai.google.dev/models");
      expect(getModelUrl("google", "gemini-1.5")).toBe("https://ai.google.dev/models");
    });
    ...
    it("should infer provider from modelId when provider is not specified", () => {
      expect(getModelUrl(undefined, "user/repo")).toBe("https://huggingface.co/user/repo");
      expect(getModelUrl(undefined, "llama2:latest")).toBe("https://ollama.com/library/llama2");
    });

    it("should use modelType to infer provider", () => {
      expect(getModelUrl(undefined, "model", "llama_model")).toBe("https://ollama.com/library/model");
    });

    it("should return null for unknown providers without modelId", () => {
      expect(getModelUrl("unknown", undefined)).toBeNull();
      expect(getModelUrl("unknown", "")).toBeNull();
    });
  });
});