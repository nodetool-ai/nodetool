 ...
 */
export function getClipboardType(contentType: string): "image" | "text" | "html" | "unsupported" {
  if (!contentType) {
    return "unsupported";
  }
...
export async function copyAssetToClipboard(
  ...
  log.info(`Copying asset to clipboard: type=${clipboardType}, contentType=${contentType}`);
 ...
 */
async function copyHtmlToClipboard(url: string, _assetName?: string): Promise<void> {
  try {
    ...
    if (window.api.clipboard?.writeHTML) {
      ...
      await window.api.clipboardWriteText(htmlContent);
...
async function copyTextToClipboard(
  ...
  try {
    ...
    if (window.api.clipboard?.writeText) {
      ...
      // Fallback to legacy API
      await window.api.clipboardWriteText(textContent);
    }
