// forked from design-sync lib/dts.mjs — src-only (.tsx) DS: point ts-morph at
// the ui_primitives barrel so prop interfaces resolve from source.
//
// The `nodetool` package (web app) ships no .d.ts and has no `types` field, so
// the stock extractor resolves `entry` to a nonexistent web/index.d.ts and only
// globs *.d.ts — every component whose props extend MUI types (e.g.
// `TextProps extends Omit<TypographyProps,...>`) collapses to
// `[key:string]: unknown`. This fork makes the ui_primitives barrel the type
// entry/root so ts-morph loads the .tsx sources; MUI types resolve via
// moduleResolution from the hoisted node_modules.
...
// Repo-specific: the design system is the ui_primitives barrel inside the web
// app. When present it is the authoritative type entry (overrides pkgJson).
const DS_BARREL_REL = 'src/components/ui_primitives/index.ts';

export function findTypesRoot(pkgDir, pkgJson) {
  if (existsSync(join(pkgDir, DS_BARREL_REL))) return dirname(join(pkgDir, DS_BARREL_REL));
...
function projectFor(pkgDir, typesRoot) {
  ...
  // forked: the ui_primitives barrel is the authoritative type entry (the app
  // ships no .d.ts). ts-morph follows its re-exports into the .tsx sources and
  // resolves real props from there.
  const entry = existsSync(join(pkgDir, DS_BARREL_REL))
    ? join(pkgDir, DS_BARREL_REL)
